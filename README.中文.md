...
HitSV 是一个全面的结构变异检测方法，支持多种测序数据类型和分析模式，能够准确检测和基因分型基因组中的结构变异。
...
HitSV 支持多种数据的输出，包括基于纯 LRS 数据、基于纯 SRS 数据 或者 结合（低深度） LRS 数据和 SRS 数据进行混合 SV 检测。

另外，可以通过设定待检测的局部区域，来快速获取特定区域的 SV 检测结果。HitSV不提供内置的多线程并行，用户通过将全染色体划分为多个不同的区域，并行运行多个HitSV实例来实现多进程并行。

针对不同类型的三代测序数据，HitSV 提供了不同的预设参数设定，以优化检测和基因分型准确性。"ONT_Q20" 以及 "HIFI" 可以用于错误率低于1%的数据，"ERR_PRONE" 可以用于错误率在1%到6%之间的数据。
...
在一次 HitSV-call 中同时提供 LRS 和 SRS 数据集，以利用它们的互补优势：
...
HitSV 不直接支持多线程变异检测。可以通过划分染色体的不同区域，并行运行多个HitSV实例来实现多进程并行。

脚本HitSV_multy_process.sh给定了一个示例，展示了如何将整个基因组划分为24个不同的区域，并行运行24个HitSV实例来实现多进程并行变异检测。
...
当基于如下参数给出指定区间的时候（-b --FC_BED），HitSV生成特定区间的局部组装序列。该功能仅仅适用于ASM以及LRS数据集，暂时不能应用与SRS数据集，并且每个指定的区间长度不能超过50,000 bp。
...
该功能用于分析特定染色体区间内，群体样本的结构变异结构。特别是为了解决复杂结构变异（CSV）的检测问题而设立。HitSV通过直接分析群体样本的局部单倍型序列信息（局部contig），而非基于vcf，避免了生成VCF的时候，一些细节丢失的问题，从而更好检测和分析群体样本的复杂的结构变异构成。该功能仅仅适用于ASM以及LRS数据集，暂时不能应用与SRS数据集，并且每个指定的区间长度不能超过50,000 bp。
