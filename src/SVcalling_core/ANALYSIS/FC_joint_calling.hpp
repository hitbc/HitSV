...
struct FC_joint_calling_handler
{
	...
	struct CONTIG_INFO
	{
		...
		std::vector<VNTR_Range> vntr_region_l;	   // VNTR regions in the contig, NOT in ref
		std::vector<VNTR_Range> non_vntr_region_l; // non-VNTR regions in the contig, NOT in ref

		bool is_used;

		bool SV_n_sub_region_is_counted;
		...
		void clear()
		{
			...
			mapping_2_ref.init(PRESET_M2); // Reinitialize the aligner
			memset(&c_r, 0, sizeof(CONTIG_COMPARE_RST_ITEM));
			right_aln_2_ref = false;
			SV_n_sub_region_is_counted = false;
		}
		...
		bool search_REGION_WITH_MEI(int st_pos, int ed_pos, std::string &base_contig_sample_name)
		{
			...
			oss << "[search_REGION_WITH_MEI-SUMMARY];with_MEI=" << with_MEI
				<< ";INS_region_in_contig=[" << st_pos << "-" << ed_pos << "];" << "insert_contig=" << sample_name << ";base_contig=" << base_contig_sample_name << ";repeat_class_family_cat=" << repeat_class_family_cat << "\n";
...
#define KSW2_PARAM 1, 4, 18, 3, 65, 1

		bool ksw2_aln_2_ref(std::vector<uint8_t> &ref_bin)
		{
			// size similarL
			mapping_2_ref.init(PRESET_M2);
			// ASN 5:  1, 19, 39, 3, 81, 1
			// ASM 10: 1, 9, 16, 2, 41, 1
			// ASM 20: 1, 4, 6, 2, 26, 1, 60
			
			//OLD :
			//mapping_2_ref.reset_paramater(1, 6, 18, 3, 65, 1);
			//minimap2 ONT-default params
			//mapping_2_ref.reset_paramater(2, 4, 4,  2, 24, 1);
			mapping_2_ref.reset_paramater(KSW2_PARAM);
			...
			fprintf(stderr, "mapping_score: %d\n", mapping_2_ref.get_mapping_score());
			mapping_2_ref.print_mapping_score();

			if(mapping_2_ref.mapping_fail()){
				fprintf(stderr, "mapping fail\n");
							return false;
			}
			// mapping_2_ref.adjustCIGAR();
	...
	void load_contig_list_one_region_fasta(const char *FC_Contig_L, std::vector<CONTIG_INFO> &contig_l)
	{
		...
		// contig is HET or HOM?
		std::unordered_map<std::string, int> contig_id_list;
		// store name count
		for (uint i = 0; i < contig_l.size(); i++)
		{
			std::string contig_name = extractName(contig_l[i].sample_name);
			if (contig_id_list.find(contig_name) == contig_id_list.end())
				contig_id_list[contig_name] = 1;
			else
				contig_id_list[contig_name]++;
		}
		// check the HET or HOM
		for (uint i = 0; i < contig_l.size(); i++)
		{
			std::string contig_name = extractName(contig_l[i].sample_name);
			if (contig_id_list.find(contig_name) == contig_id_list.end())
				continue;
			if (contig_id_list[contig_name] == 1)
				contig_l[i].is_HOM = true;
		}
	}
							  ...
							  int MIN_GAP_LEN, int suggest_st_pos)
	{
		...
		{			
			...
			if(c_type == 0 || c_type == 7){
				if(c_length >= MIN_ANCHOR_len){
					cigar_bg_C = std::min(cigar_bg_C, cigar_ID);
					cigar_ed_C = std::max(cigar_ed_C, cigar_ID);
				}
			}
		}

		fprintf(stderr, "cigar_bg_C: %d, cigar_ed_C: %d\n", cigar_bg_C, cigar_ed_C);
		...
		for (int cigar_ID = 0; cigar_ID < n_cigar; cigar_ID++)
		{
			...
			case 1:	   // insertion
				...
				{
					if(cigar_ID >= cigar_bg_C && cigar_ID <= cigar_ed_C){
						sv_l.emplace_back(); // store the SVs
						sv_l.back().store(true, a_i, b_i, c_length, &a, &b);
					}
					else{
						fprintf(stderr, "ERROR CIGAR  INS %d ", c_length);
					}
				}
				b_i += c_length;
				break;
			case 2: // Deletions
				if (cigar_ID != 0 && cigar_ID != n_cigar - 1 && c_length >= MIN_GAP_LEN)
				{
					if(cigar_ID >= cigar_bg_C && cigar_ID <= cigar_ed_C){
						...
						fprintf(stderr, "ERROR CIGAR  DEL %d ", c_length);
					}
				}
				a_i += c_length;
				break;
...
#define sequencing_platform_HPRC 1
		...
		std::string &cigar_str_3_to_2)
	{
		...
		for (uint i = 0; i < sv_l.size(); i++)
		{
			...
			int mapping_type = -1;
			int event_len = -1;
			if (sv_l[i].insert_contig->sample_contig_id == 0)
			{ // deletion to reference, the insert contig is reference
			...
			if (true == sv_l[i].base_is_also_INS_in_ref)
			{
				...
				for (uint j = 0; j < contig_to_ref_sv_l.size(); j++)
				{
					...
					if (ref_sv.is_INS == false)
						continue;
					if (std::max(ref_sv.insert_bg, cur_sv_bg) <= std::min(ref_sv.insert_ed, cur_sv_ed))
					{ // overlap
			...
			bool is_VNTR;
			int VNTR_idx;
			int distanceToNearestVNTRRegion;
		...
		for (; it != ref_NONE_VNTR_REGION_sv_counter.end(); it++)
		{
			...
			for (uint i = 0; i < non_vntr_SV_in_ref_l.size(); i++)
			{
				if (non_vntr_SV_in_ref_l[i].ref_vntr_r_p == it->first)
				{
					...
					if (false)
						fprintf(stderr, "%s", non_vntr_SV_in_ref_l[i].get_log().c_str());
				}
			}
		}
	}
	...
	void show_all_contig(std::vector<CONTIG_INFO> &contig_l, std::string &s)
	{
		...
		{
			fprintf(stderr, "contig[%d:%s][%d:%d:%d] string %s \n",
	...
	void analyzeSubRegionClusters(std::set<std::string> &non_vntr_sv_type_sum,
								  ...
								  std::string &csv_subregion_detail,
								  std::vector<std::vector<SV_ITEM>> &sv_to_ref_l)
	{
		// main region:
		{
			...
			non_vntr_sub_region_analysis_core(cluster_sorter, contig_l, var_final,
											  full,
											  ref_contig, sub_region_detail, csv_subregion_detail, non_vntr_sv_type_sum, sv_to_ref_l);
		}
	...
	static inline int cigar_op(uint32_t c) {
		return c & 0xf;
	}
	static inline int cigar_oplen(uint32_t c) {
		return c >> 4;
	}

	void get_string_in_ref_region(uint32_t* bam_cigar, int n_cigar,
							int ref_start_aln,
							int ref_start, int ref_end,
							const std::string& query_seq,
							std::string& s_in_region) {
		s_in_region.clear();

		int ref_pos = ref_start_aln;   // 当前参考坐标
		int query_pos = 0;             // 当前查询序列坐标

		for (int i = 0; i < n_cigar; ++i) {
			uint32_t c = bam_cigar[i];
			int op = cigar_op(c);
			int len = cigar_oplen(c);

			if (op == BAM_CMATCH || op == BAM_CEQUAL || op == BAM_CDIFF) {
				// 消耗参考和查询
				int op_ref_start = ref_pos;
				int op_ref_end   = ref_pos + len;  // 半开区间
				int ovlp_start = std::max(op_ref_start, ref_start);
				int ovlp_end   = std::min(op_ref_end,   ref_end);

				if (ovlp_start < ovlp_end) {
					int q_start = query_pos + (ovlp_start - op_ref_start);
					int q_len   = ovlp_end - ovlp_start;
					s_in_region.append(query_seq, q_start, q_len);
				}
				...
				if (ref_pos >= ref_start && ref_pos < ref_end) {
					s_in_region.append(query_seq, query_pos, len);
				}
				query_pos += len;      // ref_pos 不变

			} else if (op == BAM_CDEL || op == BAM_CREF_SKIP) {
				// 缺失 / 跳过参考内含子：消耗参考，不消耗查询
				ref_pos += len;

			} else if (op == BAM_CSOFT_CLIP) {
				// 软剪切：消耗查询，不消耗参考，不属于比对区域，跳过
				query_pos += len;

			} else if (op == BAM_CHARD_CLIP) {
				// 硬剪切：不消耗查询（不在序列中），不消耗参考，忽略

			} else {
				// 其他操作（如 P 等）极少出现，保守处理：不输出，适当移动坐标
				// P (padding) 在规范中消耗参考但不消耗查询，这里按缺失处理
				// 若未来有特殊需求可在此扩展
				ref_pos += len;
			}
		}
										   ...
										   std::vector<std::vector<SV_ITEM>> &sv_to_ref_l)
	{
		...
		std::string sv_N_contig_number_s;
		std::string sv_region_header =  "Subregion_" + (vr.id == -1 ? "FULL" : std::to_string(vr.id)) + "_" + std::to_string(vr.start) + "-" + std::to_string(vr.end);
		std::string sv_region_contig_s;

		// store the SV number in each VNTR/NON-VNTR region:
		{
			int total_contig_number = 0;
			int with_SV_contig_number = 0;
			int with_CSV_contig_number = 0;

			int ref_len = contig_l[0].contig.size();
			// for each contig
			//  contig_noref_id = contig_id - 1
			for (int contig_id = 0; contig_id < sv_to_ref_l.size(); contig_id++)
			{
				CONTIG_INFO &c_q = contig_l[contig_id];
				if (c_q.is_used == false)// skip the contig that is not used: NOT ALIGNED 
					continue;
				if (false == c_q.SV_n_sub_region_is_counted)
					continue;
				std::vector<SV_ITEM> &sv_item_l = sv_to_ref_l[contig_id];
				int sv_num_in_region = 0;
				std::string svcontig_id_s;
				// for each sv in the contig
				for (size_t j = 0; j < sv_item_l.size(); j++)
				{
					bool is_SV_in_region = false;
					SV_ITEM &sv_item = sv_item_l[j];
					if (sv_item.is_INS)
					{
						if (ref_len - sv_item.base_ed < 50 || sv_item.base_bg < 50) // At least 50bp away from the end of the ref
							continue;
						if (vr.with_in_region(sv_item.base_bg) || vr.with_in_region(sv_item.base_ed))
							is_SV_in_region = true;
					}
					else
					{
						if (ref_len - sv_item.insert_ed < 50 || sv_item.insert_bg < 50) // At least 50bp away from the end of the ref
							continue;
						if (vr.with_in_region(sv_item.insert_bg) || vr.with_in_region(sv_item.insert_ed))
							is_SV_in_region = true;
					}
					if (is_SV_in_region){
						//show the SV in the region
						sv_num_in_region++;
						svcontig_id_s += (std::to_string(sv_item.base_STR.size()) + "->" + std::to_string(sv_item.insert_STR.size()) + "^");
					}
				}

				int add_hap_n = (c_q.is_HOM ? 2 : 1);
				total_contig_number += add_hap_n;

				if (sv_num_in_region > 0)
					with_SV_contig_number += add_hap_n;
				if (sv_num_in_region > 1)
					with_CSV_contig_number += add_hap_n;
				//is CSV
				if (1){
					//get the contig string in this region
					uint32_t* bam_cigar = NULL;
					int n_cigar = c_q.mapping_2_ref.get_cigar(&bam_cigar);
					std::string s_in_region;
					get_string_in_ref_region(bam_cigar, n_cigar, 0, vr.start, vr.end + 1, c_q.contig, s_in_region);
					//
					sv_region_contig_s += "region=" + sv_region_header + ";SAMPLE=" + c_q.sample_name + ";SV_NUM="+std::to_string(sv_num_in_region)+";CON=" + s_in_region + ";SV=" + svcontig_id_s + "\n";
				}
			}
			sv_N_contig_number_s += " total_contig_number: " + std::to_string(total_contig_number) + " with_SV_contig_number: " + std::to_string(with_SV_contig_number) + " with_CSV_contig_number: " + std::to_string(with_CSV_contig_number) + "\n";
		}
		csv_subregion_detail += sv_region_contig_s;
		// store sub region results
		store_sub_region_result(non_vntr_sv_type_sum_size, sv_type_count_s, ref_contig, REF_VNTR_R_K.c_str(), sub_cluster_all_contig, sub_cluster_all_ori_clu,
								sub_region_detail, sv_N_contig_number_s, is_CSV_sub_region,
	...
	void store_sub_region_result(std::string &non_vntr_sv_type_sum_size,
								 ...
								 bool is_CSV_sub_region, std::vector<int> &type_VINV_GREEN_sv_base_absolute_BP_in_ref, int &clu_n_with_ONLY_Insert_VNTR_svs,
								 int vr_len)
	{
						   ...
						   std::vector<std::vector<SV_ITEM>> &sv_to_ref_l)
	{
		...
		for (int contig_query_id = 0; contig_query_id < contig_n && contig_query_id < max_used_contig; contig_query_id++)
		{
			...
			// store the flag: whether the contig variants is used in each VNTR/NON-VNTR region:
			if (contig_query_id > 0)
			{
				c_q.SV_n_sub_region_is_counted = true;
			}
		}
		...
		std::string csv_subregion_detail = "csv_subregion_detail\n";
		analyzeSubRegionClusters(non_vntr_sv_type_sum, cluster_sorter, contig_l, ref_contig, var_final, sub_region_detail, csv_subregion_detail, sv_to_ref_l);
		...
		// LOG: csv_subregion_detail
		file_name = "csv_subregion_detail.log";
		store_main_log_simple(work_dir, file_name, csv_subregion_detail);
	...
	int align_main(int argc, char *argv[])
	{
		if (argc < 8)
		{
			std::cerr << "Usage: " << argv[0] << " <target_seq> <query_seq> <match> <mismatch> <gap_open> <gap_extend> <gap_open2> <gap_extend2> <output_mode>" << std::endl;
			std::cerr << "output_mode: FULL, ALN or CIGAR" << std::endl;
			return 1;
		}

		const char *target_seq_p = argv[1];
		const char *query_seq_p = argv[2];
		int match_D = atoi(argv[3]);
		int mismatch_D = atoi(argv[4]);
		int gap_open_D = atoi(argv[5]);
		int gap_ex_D = atoi(argv[6]);
		int gap_open2_D = atoi(argv[7]);
		int gap_ex2_D = atoi(argv[8]);
		const char * output_mode = argv[9];

		std::string target_seq_s(target_seq_p);
		std::string query_seq_s(query_seq_p);

		// Convert sequences to binary format
		std::vector<uint8_t> target_bin, query_bin;
		store_bin_contig(target_seq_s, target_bin);
		store_bin_contig(query_seq_s, query_bin);
		...
		if (strcmp(output_mode, "FULL") == 0)
		{
			ca.printf_alignment_detail(stdout, 0, NULL, 0);
		}
		else if (strcmp(output_mode, "ALN") == 0)
		{
			ca.print_X_E_M2(stdout, 0);
		}
		else if (strcmp(output_mode, "CIGAR") == 0)
		{
			std::cout << ca.printCIGAR_core() << std::endl;
		}
		else
		{
			xassert(0, "Fatal error: output_mode must be FULL, ALN, or CIGAR");
		}
		return 0;
	}
	...
	int ana_main(int argc, char *argv[])
	{
		...
		if (argc < 5)
		{
			fprintf(stderr, "Error: Insufficient arguments for FC_joint_ana\n");
			fprintf(stderr, "Usage: tools FC_joint_ana [FC_Contig_fasta_L] [RM_L] [TRF_L] [sequencing_platform]\n");
			fprintf(stderr, "Example: tools FC_joint_ana  contigs.fasta RM.out TRF.out 0 \n");
			fprintf(stderr, "  FC_Contig_fasta_L: File containing contig list in FASTA format\n");
			fprintf(stderr, "  RM_L: RepeatMasker result file\n");
			fprintf(stderr, "  TRF_L: TRF result file\n");
			fprintf(stderr, "  sequencing_platform: Sequencing platform (HPRC or ONT)\n");
			return 1;
		}

		std::string work_dir = "./";
		const char *FC_Contig_fasta_L = argv[1];
		const char *RM_L = argv[2];
		const char *TRF_L = argv[3];

		int sequencing_platform = 0;
		if (strcmp(argv[4], "HPRC") == 0)
		{
			sequencing_platform = sequencing_platform_HPRC;
		}
		else if (strcmp(argv[4], "ONT") == 0)
		{
			sequencing_platform = sequencing_platform_ONT;
		}
		else
		{
			xassert(0, "Fatal error: dataset must be one of HPRC or ONT");
		}

		Contig_String_aligner ca;
		ca.init(PRESET_M2);
		//1, 19, 39, 3, 81, 1,
		ca.reset_paramater(KSW2_PARAM);
		...
		xassert(contig_l.size() == sv_to_ref_l.size(), "The number of contigs and SV lists do not match");
