...
struct Genomic_context_Analysis_handler{
	struct SV_basic_infomation{
		...
		int SV_length;
		int maxVNTR_value;
		int isHIGH;//is HIGH confidence SV, when the SV is HIGH confidence, it is true.
		...
		void store_SRS_bam_sig_data(bool is_CIGAR, bool is_CLIP, bool is_DRP, int is_SMALL_VAR){
			if(is_CIGAR) this->SIG_CIGAR_N++;
			if(is_CLIP)  this->SIG_CLIP_N++;
			if(is_DRP) 	 this->SIG_DRP_N++;
			if(is_SMALL_VAR)  this->SIG_SMALL_VAR_N++;
			this->Read_N++;
		}
	};

	struct LRS_SMALL_SIG_HANDLER_GCA{
		...
		bool store_sig(bool print_log, int chrID,  int POS, int influence_base, int & middle_pos){
	...
	void init_the_counter(std::vector<REF_BASIC_DATA> &reference_list,
	...
	void load_vcf_data_file(std::vector<REF_BASIC_DATA> &reference_list, char * vcf_fn_in, int ana_block_length, int ana_block_step_size){
		...
		do{
			...
			if(0){
				sv_END_r = vcf_get_sv_END(vcf_read.header, c_r, &SV_END);
				SV_length_r = vcf_get_sv_LENGTH(vcf_read.header, c_r, &SV_length);
				sv_type_r = vcf_get_sv_type(vcf_read.header, c_r, c_sv_type);
			}

			if(sv_END_r == -1 || SV_length_r == -1 || sv_type_r == -1){
				//get ALT len

				//get ALT len
				int n_allele = c_r->n_allele;      // 等位基因数：REF+ALT
				char **allele = c_r->d.allele;     // allele[0]=REF, allele[1]=ALT1, ...
				xassert(n_allele >= 2, "n_allele must be >= 2");
				const char *REF = allele[0];
				const char *ALT  = (n_allele>1) ? allele[1] : NULL;

				int ref_len = strlen(REF);    // REF 长度
				int alt_len = strlen(ALT);    // ALT 长度

				// 1. SV 长度 = ALT 长度 - REF 长度（标准SV长度定义）
				SV_length = alt_len - ref_len;

				// 2. END = POS + REF 长度（VCF 标准定义）
				SV_END = c_r->pos + ref_len;

				// 3. SV 类型（直接从 ALT 判断）
				if (SV_length > 0) {
					c_sv_type = "INS";  // INS 插入
				} else if (SV_length < 0) {
					c_sv_type = "DEL";  // DEL 缺失
					SV_length = -SV_length;  // 转成正数长度
				} else {
					c_sv_type = "Other";  // 其他类型
				}
			}

			if(SV_length > - min_SV_LEN && SV_length < min_SV_LEN){
				continue;
			}
			//fprintf(stderr, "%d\t%d\t%d\t%d\t%s\n",SV_CHR_ID,SV_POS,SV_END,SV_length,c_sv_type);
			//fprintf(stderr, "%d\t%d\t%d\n",sv_END_r,SV_length_r,sv_type_r);
	...
	void print_help_genomic_contest_Analysis_BAM(const char* program_name) {
		std::cerr << "========================================" << std::endl;
		std::cerr << "Usage: " << PACKAGE_NAME << " tools " << program_name << " <ref_fn> <input_bam_fn> <ana_block_length> <ana_block_step_size>" << std::endl;
		std::cerr << "========================================" << std::endl;
		std::cerr << "Parameters:" << std::endl;
		std::cerr << "  <ref_fn>              Path to the reference file (e.g., .fa/.fasta)" << std::endl;
		std::cerr << "  <input_bam_fn>        Path to the input BAM file" << std::endl;
		std::cerr << "  <ana_block_length>    Length of the analysis block (must be a positive integer)" << std::endl;
		std::cerr << "  <ana_block_step_size> Step size of the analysis block (must be a positive integer)" << std::endl;
		std::cerr << "========================================" << std::endl;
		std::cerr << "Example:" << std::endl;
		std::cerr << "  " << program_name << " ref.fa input.bam 600 300" << std::endl;
		std::cerr << "========================================" << std::endl;
	}

	void print_help_genomic_contest_Analysis_VCF(const char* program_name) {
		std::cerr << "========================================" << std::endl;
		std::cerr << "Usage: " << PACKAGE_NAME << " tools " << program_name << " <ref_fn> <vcf_fn_in> <input_VNTR_region_file> <HIGH_CONFIDENCE_region_file>" << std::endl;
		std::cerr << "========================================" << std::endl;
		std::cerr << "Parameters:" << std::endl;
		std::cerr << "  <ref_fn>                       Path to the reference file (e.g., .fa/.fasta)" << std::endl;
		std::cerr << "  <vcf_fn_in>                    Path to the input VCF file(s), separate by ','" << std::endl;
		std::cerr << "  <input_VNTR_region_file>       Path to the input VNTR region file" << std::endl;
		std::cerr << "  <HIGH_CONFIDENCE_region_file>  Path to the high confidence region file" << std::endl;
		std::cerr << "========================================" << std::endl;
		std::cerr << "Example:" << std::endl;
		std::cerr << "  " << PACKAGE_NAME << " tools " << program_name << " ref.fa input.vcf input.bam vntr.bed high_confidence.bed" << std::endl;
		std::cerr << "========================================" << std::endl;
	}
	...
	struct SA_INFO{
		...
		void show(){
			std::cerr << chrom_ID << "\t" << pos << "\t" << direction << "\t" << mapq << "\t" << cigar << "\t" << next << "\t" << break_point_left << "\t" << break_point_right << "\t" << soft_sa_left << "\t" << soft_sa_right << std::endl;
		}

		void SA_cigar_break_point()
		{
			//string to path
			path_segment path_a[100];
			path_t path = {0};
			path.m = 100; path.a = path_a;
			char_cigar_to_path(cigar, &path);

			soft_sa_left = soft_sa_right = break_point_left = break_point_right = 0;
			//init
			...
			{
				soft_sa_left = path_a[0].length;
				break_point_left = pos;
			}
			...
			if(	end_type == CIGAR_SOFT_CLIP ||	end_type == CIGAR_HARD_CLIP)
			{
				...
				break_point_right = pos + match_delete_len;
				soft_sa_right = path_a[path.n - 1].length;
			}
		}

		bool bam_get_sa(bam1_t* b, bam_hdr_t *h)
		{
			char* SA_tag_char = bam_get_string_tag(b, "SA");
			if(SA_tag_char == NULL)
				return false;
			char SA_tag_char_cp[1024], *char_p = SA_tag_char_cp;
			if(strlen(SA_tag_char) > 1023)
				return false;
			...
			char chr_ID[200];
			char direction;
			sscanf(SA_tag_char_cp, "%s %d %c %s %*d %d", chr_ID, &(pos), &direction, cigar, &(mapq));
			chrom_ID = bam_name2id(h, chr_ID);
			direction = (direction == '+')?FORWARD:REVERSE;
			next = 0;
			SA_cigar_break_point();
			if(break_point_left == 0 && break_point_right == 0)
				return false;
			return true;
		}
	};
	...
	private:
		faidx_t *ref_idx;
		char *current_chr_seq;
		int current_chr_id;
		char *ref_fn;
		int current_chr_length;

	public:
		ReferenceGenomeLoader(const char *ref_filename) {
			ref_fn = strdup(ref_filename);
			ref_idx = reference_index_load(ref_fn);
			current_chr_seq = NULL;
			current_chr_id = -1;
			current_chr_length = 0;
		}
		
		~ReferenceGenomeLoader() {
			if (current_chr_seq != NULL) {
				free(current_chr_seq);
			}
			if (ref_idx != NULL) {
				fai_destroy(ref_idx);
			}
		...
		char* get_chromosome_sequence(int chr_id) {
			if (chr_id == current_chr_id) {
				return current_chr_seq;
			}
			...
			// 获取新染色体的序列
			const char *chr_name = faidx_iseq(ref_idx, chr_id);
			current_chr_length = faidx_seq_len(ref_idx, chr_name);
			
			// 分配内存并读取序列
			int load_len = 0;
			current_chr_seq = faidx_fetch_seq(ref_idx, chr_name, 0, current_chr_length - 1, &load_len);
			
			current_chr_id = chr_id;
			return current_chr_seq;
		}
		
		int get_chromosome_length(int chr_id) {
			if (chr_id == current_chr_id) {
				return current_chr_length;
			}
			
			const char *chr_name = faidx_iseq(ref_idx, chr_id);
			return faidx_seq_len(ref_idx, chr_name);
		}
		
		int get_current_chr_id() {
			return current_chr_id;
		}
	...
	int genomic_contest_Analysis_SRS(int argc, char *argv[]){

		if (argc != 5) {
			std::cerr << "Error: Incorrect number of arguments." << std::endl;
			print_help_genomic_contest_Analysis_BAM(argv[0]);
			return EXIT_FAILURE;
		}
		...
		int ana_block_length = atoi(argv[3]);
		int ana_block_step_size = atoi(argv[4]);

		std::vector<REF_BASIC_DATA> reference_list;
		faidx_t *c_ref_idx = reference_index_load(ref_fn);
		ReferenceGenomeLoader ref_loader(ref_fn);

		init_the_counter(reference_list, ana_block_length, ana_block_step_size, c_ref_idx);
		//open vcf files
		Bam_file c_b;
		bam_file_open(input_bam_fn, ref_fn, NULL, &c_b);
		bam_hdr_t *header = c_b._hdr;
		...
		//store data:
		{
			if(0){//debug code 
				R_region region;
				region.chr_ID = 1;
				region.st_pos = 50370434 - 100000;
				region.ed_pos = 50370434 + 100000;
				//
				resetRegion_ID(&c_b, &region); 
			}
			...
			while (bam_next(&c_b)) {
				...
				if(read_count % 10000 == 0){
					std::cerr << "read_count: " << read_count << " ";
					std::cerr << "read_POS: " << b->core.tid << ":" << b->core.pos << std::endl;
				}
				...
				xassert(read_len <= 512, "Error: read_len is too long.");
				...
				//
				//SNP and INDEL
				bool is_SMALL_VAR = false;
				is_SMALL_VAR = is_dense_cigar(b, ref_loader.get_chromosome_sequence(b->core.tid));
				...
				int match_LEN = 0;
				{
					int INDEL_len = 0;
					int NM_len = 0;
					match_LEN = bam_get_match_NM(b,INDEL_len, NM_len);
					if(INDEL_len >= 30) 
						is_CIGAR = true; 
				}
				...
				if(1){
					SA_INFO sa;
					bool is_sa = sa.bam_get_sa(b, header);
					//show sa
					if(is_sa && sa.mapq > 10 && sa.chrom_ID == b->core.tid && ABS(sa.pos - b->core.pos ) < 50000){
						is_CIGAR = true; 
						//printf("SA: %s %d, ", bam_get_qname(b), match_LEN);
						//sa.show();
					}
				}

				//MAPQ
				bool is_low_MAPQ = false;
				{
					uint8_t mapq = b->core.qual;
					if(mapq < 20)	is_low_MAPQ = true;
				}

				//which region the read belong to?
				int tid = b->core.tid;
				int pos = b->core.pos;
				int end = pos + read_len - 1;
				int middle_pos = (pos + end) / 2;

				ana_block_length, ana_block_step_size;
				int store_block_id1 = 0, store_block_id2 = 0;
				get_block_ID_by_pos(store_block_id1, store_block_id2, middle_pos, ana_block_length, ana_block_step_size);

				xassert(tid >= 0 && tid < reference_list.size(), "Error: tid is out of range.");
				xassert(store_block_id1 < reference_list[tid].region_withSV.size() && store_block_id1 >= 0, "Error: store_block_id1 is out of range.");
				xassert(store_block_id2 < reference_list[tid].region_withSV.size() && store_block_id2 >= 0, "Error: store_block_id2 is out of range.");

				reference_list[tid].region_withSV[store_block_id1].store_SRS_bam_sig_data(is_CIGAR, is_clip, is_DRP, is_SMALL_VAR);
				reference_list[tid].region_withSV[store_block_id2].store_SRS_bam_sig_data(is_CIGAR, is_clip, is_DRP, is_SMALL_VAR);

			}
		}

		//output the results:
		out_put_bam_analysis_result(c_ref_idx, reference_list, ana_block_length, ana_block_step_size);
	...
	void out_put_bam_analysis_result(faidx_t *c_ref_idx, std::vector<REF_BASIC_DATA> &reference_list, int ana_block_length, int ana_block_step_size){
		int region_ID = -1;
		std::vector<std::string> item_value;
		//load reference index
		int N_seq = faidx_nseq(c_ref_idx);
		N_seq = MIN(24, N_seq);
		//print the header:
		printf("chrID\tpos_begin\tregion_ID\tSIG_CIGAR_N\tSIG_CLIP_N\tSIG_DRP_N\tSIG_SMALL_VAR_N\n");
		for(int i = 0; i < N_seq; i++){
			...
			for(SV_basic_infomation & sr: u.region_withSV){
				...
				printf("%d\t%d\t%d\t", u.chrID, sr.pos_begin, sr.pos_begin + 600);
				printf("%d\t%d\t%d\t%d\t", sr.SIG_CIGAR_N, sr.SIG_CLIP_N, sr.SIG_DRP_N, sr.SIG_SMALL_VAR_N);
				//16
				printf("\n");
			}
		}
	...
	void add_LRS_sig_core(std::vector<REF_BASIC_DATA> &reference_list, char SIG_type, int tid, int store_block_id1){

		switch (SIG_type)
		{
		case 'C': //CIGAR
			reference_list[tid].region_withSV[store_block_id1].SIG_CIGAR_N++;
			break;
		case 'D': //DRP
			reference_list[tid].region_withSV[store_block_id1].SIG_DRP_N++;
			break;
		case 'S': //SMALL_VAR	
			reference_list[tid].region_withSV[store_block_id1].SIG_SMALL_VAR_N++;
			break;	
		case 'E': //CLIP	
			reference_list[tid].region_withSV[store_block_id1].SIG_CLIP_N++;
			break;	
		default:
			break;
		}
	...
	void add_LRS_sig(std::vector<REF_BASIC_DATA> &reference_list, int tid, int breakpoint, int ana_block_length, int ana_block_step_size, char SIG_type){
		int store_block_id1 = 0, store_block_id2 = 0;
		get_block_ID_by_pos(store_block_id1, store_block_id2, breakpoint, ana_block_length, ana_block_step_size);
		add_LRS_sig_core(reference_list, SIG_type, tid, store_block_id1);
		if(store_block_id2 != store_block_id1){
			add_LRS_sig_core(reference_list, SIG_type, tid, store_block_id2);
		}
	}

	int genomic_contest_Analysis_LRS(int argc, char *argv[]){
		if (argc != 5) {
			std::cerr << "Error: Incorrect number of arguments." << std::endl;
			print_help_genomic_contest_Analysis_BAM(argv[0]);
			return EXIT_FAILURE;
		...
		init_the_counter(reference_list, ana_block_length, ana_block_step_size, c_ref_idx);
		LRS_SMALL_SIG_HANDLER_GCA small_sig_queue_handler;
		...
		path_segment* path = (path_segment* )calloc(100000, sizeof(path_segment));
		{
			std::vector<uint8_t> storeReadBuff;
			//set the region:
			int read_ID = 0;
			while (bam_next(&c_b)) {
				small_sig_queue_handler.clear();

				bam1_t *br = &(c_b._brec);
				read_ID ++;
				if(read_ID % 3000 == 0){
					fprintf(stderr, "r_id=%d, tid=%d, pos=%d\n", read_ID, br->core.tid, br->core.pos);
				}
				...
				xassert(n_cigar <= 100000, "n_cigar > 100000");
				for (unsigned int i = 0; i < n_cigar; ++i){
					path[i].length = (bam_cigar[i] >> BAM_CIGAR_SHIFT);
					path[i].type   = (int)(1 + (bam_cigar[i] & BAM_CIGAR_MASK));
				}
				//get read sequence
				storeReadBuff.resize(br->core.l_qseq);
				get_bam_seq_bin(0, br->core.l_qseq, storeReadBuff.data(), br);
				uint8_t *qseq = storeReadBuff.data();
				//get reference sequence
				char *tseq = ref_loader.get_chromosome_sequence(br->core.tid) + br->core.pos;
				...
				for(int i = 0; i < n_cigar; i++){
					...
					{
						if(p->length >= 200){//at lease 200 bp
							...
							//store CLIP signals
							add_LRS_sig(reference_list, tid, breakpoint, ana_block_length, ana_block_step_size, 'E');
						}
						seq_i += p->length;
					}
						...
						if(p->length >= (unsigned)MIN_GCA_sv_len){
							...
							int tid = br->core.tid;
							int breakpoint = br->core.pos + ref_i;
							add_LRS_sig(reference_list, tid, breakpoint, ana_block_length, ana_block_step_size, 'C');
						}else{
							//store small sigs
							int middle_pos = 0;
							if(true == small_sig_queue_handler.store_sig(print_log, br->core.tid, br->core.pos + ref_i, p->length, middle_pos)){
								add_LRS_sig(reference_list, br->core.tid, middle_pos, ana_block_length, ana_block_step_size, 'S');
							}
							//;
						}
						seq_i += p->length;
					...
					else if(p->type == align_t::CIGAR_DELETE)
					{
						if(p->length >= (unsigned)MIN_GCA_sv_len){
							if(print_log) fprintf(stderr, "BIG_SIG:idx:%d TID: %d POS %d LEN:%d TYPE:%c @ NAME %s \n",
									i - cigar_idx, br->core.tid, br->core.pos + ref_i, p->length, pathSTR[p->type], bam_get_qname(br));
							int tid = br->core.tid;
							add_LRS_sig(reference_list, tid, br->core.pos + ref_i, ana_block_length, ana_block_step_size, 'C');
							add_LRS_sig(reference_list, tid,  br->core.pos + ref_i + p->length, ana_block_length, ana_block_step_size, 'C');
						}else{
							int middle_pos = 0;
							if(true == small_sig_queue_handler.store_sig(print_log, br->core.tid, br->core.pos + ref_i, p->length, middle_pos)){
								add_LRS_sig(reference_list, br->core.tid, middle_pos, ana_block_length, ana_block_step_size, 'S');
						...
						for(uint i = 0; i < p->length; i++, ref_i++, seq_i++){
							if("ACGTNNNNNNNNN"[qseq[seq_i]]  != tseq[ref_i]){//mis match 
								int middle_pos = 0;
								if(true == small_sig_queue_handler.store_sig(print_log, br->core.tid, br->core.pos + ref_i, p->length, middle_pos)){
									add_LRS_sig(reference_list, br->core.tid, middle_pos, ana_block_length, ana_block_step_size, 'S');
								}
	...
	int genomic_contest_Analysis_ANNO(int argc, char *argv[]){

		if (argc != 5) {
			std::cerr << "Error: Incorrect number of arguments." << std::endl;
			print_help_genomic_contest_Analysis_VCF(argv[0]);
			return EXIT_FAILURE;
		}
		...
		char * input_VNTR_region_file = argv[3];
		char * HIGH_CONFIDENCE_region_file = argv[4];
		...
		{
			//print the header:
			printf("chrID\tstart\tend\tSVType\tregion_ID\tRepeat\tisHIGH\n");
			int region_ID = -1;
			std::vector<std::string> item_value;
			//load reference index
			int N_seq = faidx_nseq(c_ref_idx);
			...
			for(int i = 0; i < N_seq; i++){
				...
				for(SV_basic_infomation & sr: u.region_withSV){
					...
					printf("%d\t%d\t%d\t", u.chrID, sr.pos_begin, sr.pos_begin + ana_block_length);
					printf("%s\t%d\t", sr.SVType.c_str(), region_ID);
					printf("%d\t%d\t", sr.maxVNTR_value, sr.isHIGH);
					printf("\n");
				}
			}
		}
	...
	bool is_dense_cigar(bam1_t *b, char *ref_char){
		std::vector<int> mismatch_position;
		//SNP and INDEL
		{
			int INDEL_len = 0;
	...
	int bam_get_match_NM(bam1_t *b, int &INDEL_SIZE, int &NM_SIZE)
	{
		int INDEL_len = 0;
		int INDEL_number = 0;
		int match = 0;
		...
		for (int i = 0; i < b->core.n_cigar; i++)
		{
			...
			if (type == CIGAR_MATCH)
				match += (bam_cigar[i] >> BAM_CIGAR_SHIFT);
			else if (type == CIGAR_INSERT || type == CIGAR_DELETE)
			{
				INDEL_number++;
				INDEL_len += (bam_cigar[i] >> BAM_CIGAR_SHIFT);
			}
		}
		...
		//number of mismatch/insert/delete events
		NM_SIZE = NM - INDEL_len + INDEL_number;
		return match;
	}
...
int randomGenerateSV(int argc, char *argv[]){
			...
			fprintf(fp_out, "##INFO=<ID=SVTYPE,Number=1,Type=String,Description=\"Type of structural variant\">\n");
			fprintf(fp_out, "##INFO=<ID=END,Number=1,Type=Integer,Description=\"End position of the structural variant described in this record\">\n");
			fprintf(fp_out, "##INFO=<ID=SVLEN,Number=.,Type=Integer,Description=\"Difference in length between REF and ALT alleles\">\n");
			fprintf(fp_out, "##INFO=<ID=SIM_INS_ST,Number=.,Type=Integer,Description=\"Random insertion string original position.(compared with POS)\">\n");
			fprintf(fp_out, "##INFO=<ID=SVANN,Number=.,Type=String,Description=\"Repeat annotation of structural variant\">\n");
			fprintf(fp_out, "##INFO=<ID=CIPOS,Number=2,Type=Integer,Description=\"Confidence interval around POS for imprecise variants\">\n");
			fprintf(fp_out, "##INFO=<ID=MATEID,Number=.,Type=String,Description=\"ID of mate breakends\">\n");
			fprintf(fp_out, "##INFO=<ID=MATEDIST,Number=1,Type=Integer,Description=\"Distance to the mate breakend for mates on the same contig\">\n");
			fprintf(fp_out, "##INFO=<ID=IMPRECISE,Number=0,Type=Flag,Description=\"Imprecise structural variation\">\n");
			fprintf(fp_out, "##INFO=<ID=SHADOWED,Number=0,Type=Flag,Description=\"CNV overlaps with or is encapsulated by deletion\">\n");
			fprintf(fp_out, "##ALT=<ID=INV,Description=\"Inversion\">\n");
			fprintf(fp_out, "##ALT=<ID=DUP,Description=\"Duplication\">\n");
			fprintf(fp_out, "##ALT=<ID=CNV,Description=\"Copy number variable region\">\n");
			fprintf(fp_out, "##FILTER=<ID=Decoy,Description=\"Variant involves a decoy sequence\">\n");
			fprintf(fp_out, "##FILTER=<ID=NearReferenceGap,Description=\"Variant is near (< 1000 bp) from a gap (run of >= 50 Ns) in the reference assembly\">\n");
			fprintf(fp_out, "##FILTER=<ID=NearContigEnd,Description=\"Variant is near (< 1000 bp) from the end of a contig\">\n");
			fprintf(fp_out, "##FILTER=<ID=InsufficientStrandEvidence,Description=\"Variant has insufficient number of reads per strand (< 0).\">\n");
			fprintf(fp_out, "##FILTER=<ID=NotFullySpanned,Description=\"Duplication variant does not have any fully spanning reads.\">\n");
			fprintf(fp_out, "##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">\n");
			fprintf(fp_out, "##FORMAT=<ID=AD,Number=R,Type=Integer,Description=\"Read depth per allele\">\n");
			fprintf(fp_out, "##FORMAT=<ID=DP,Number=1,Type=Integer,Description=\"Read depth at this position for this sample\">\n");
			fprintf(fp_out, "##FORMAT=<ID=SAC,Number=.,Type=Integer,Description=\"Number of reads on the forward and reverse strand supporting each allele including reference\">\n");
			fprintf(fp_out, "##FORMAT=<ID=CN,Number=1,Type=Integer,Description=\"Copy number genotype for imprecise events\">\n");
			fprintf(fp_out, "##reference=file:%s\n", ref_fn);
			for(int i = 0; i < n_targets; i++){
				fprintf(fp_out, "##contig=<ID=%s,length=%d>\n", header->target_name[i], header->target_len[i]);
			}
			fprintf(fp_out, "#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO	FORMAT	demo_SAMPLE\n");
			...
			for(int chr_ID = 0; chr_ID < n_targets; chr_ID++){
				...
				for(int sv_ID = 0; sv_ID < del_num; sv_ID++){
					...
					fprintf(fp_out, "%s\t%d\trandom.DEL.%d\t", header->target_name[chr_ID], st_pos, del_ID++);
					for(int i = 0; i < length + 1; i++){
						fprintf(fp_out, "%c", ref_seq.seq.s[st_pos + i - 1]);
					}
					fprintf(fp_out, "\t%c\t.\tPASS\tSVTYPE=DEL;END=%d;SVLEN=%d\tGT:AD:DP\t1/1:0,31:31\n", ref_seq.seq.s[st_pos - 1], st_pos + length, length);
				}
				//INS:
				for(int sv_ID = 0; sv_ID < ins_num; sv_ID++){
					...
					xassert(length < 3000, "");
					if(ref_seq.seq.s[st_pos] == 'N') { sv_ID--; continue; }
					fprintf(fp_out, "%s\t%d\trandom.INS.%d\t", header->target_name[chr_ID], st_pos , ins_ID++);
					fprintf(fp_out, "%c\t%c", ref_seq.seq.s[st_pos - 1], ref_seq.seq.s[st_pos - 1]);
					int insert_seq_st = 0;
					do{
						int levle = 0;
					...
					for(int i = 0; i < length; i++){
						fprintf(fp_out, "%c", ref_seq.seq.s[st_pos + insert_seq_st + i - 1]);
					}
					fprintf(fp_out, "\t.\tPASS\tSVTYPE=INS;END=%d;SIM_INS_ST=%d;SVLEN=%d\tGT:AD:DP\t1/1:0,31:31\n", st_pos, insert_seq_st, length);
				}
			}
	...
	//			heat_map[YValue_idx][mappibility_idx].region_BLANK++;
		...
		fprintf(fp_out, "heatMapData\t");
		for(uint i = 0; i < HEAT_COL_NUM; i++){
			fprintf(fp_out, "M_%d\t",i);
		}
		fprintf(fp_out, "\n");
		uint i = 0;
		//P_log
		for(std::vector<HAET_MAP_RST> & hr: heat_map){
			fprintf(fp_out, "Y_%d\t", i++);
		...
		for(std::vector<HAET_MAP_RST> & hr: heat_map){
			...
			for(HAET_MAP_RST & h: hr){
				...
				fprintf(fp_out, "%d\t", total_region_num);
			}
			fprintf(fp_out, "\n");
		}
		fprintf(fp_out, "\n\n\n");
		...
		for(std::vector<HAET_MAP_RST> & hr: heat_map){
			fprintf(fp_out, "Y_%d\t", i++);
			for(HAET_MAP_RST & h: hr){
				fprintf(fp_out, "%d\t", h.region_BLANK);
			}
			fprintf(fp_out, "\n");
		}
		fprintf(fp_out, "\n\n\n");
	...
	int heatMapDataSum(int argc, char *argv[]){
		...
		for(int i = 0; i < 100; i++){
			...
			for(int j = 0; j < 200; j++){
				...
				//	fprintf(fp_out, "0\t");
				//else
					fprintf(fp_out, "%ld\t", result_1[i][j]);
			}
			fprintf(fp_out, "\n");
		}
		fprintf(fp_out, "\n");
		...
		for(int i = 0; i < 100; i++){
			...
			for(int j = 0; j < 200; j++){
				...
				//	fprintf(fp_out, "0\t");
				//else
					fprintf(fp_out, "%ld\t", result_2[i][j]);
			}
			fprintf(fp_out, "\n");
		}
		return 0;
