...
//GC analysis---------------------------------------------------------
int genomic_contest_Analysis_SRS_run(int argc, char *argv[]){
	Genomic_context_Analysis_handler r;
	return r.genomic_contest_Analysis_SRS(argc, argv);
}

int genomic_contest_Analysis_LRS_run(int argc, char *argv[]){
	Genomic_context_Analysis_handler r;
	return r.genomic_contest_Analysis_LRS(argc, argv);
}

int genomic_contest_Analysis_ANNO_run(int argc, char *argv[]){
	Genomic_context_Analysis_handler r;
	return r.genomic_contest_Analysis_ANNO(argc, argv);
}
...
int analysis_main(int argc, char *argv[])
{
	...
	ch.add_function("csv_region_500w", "usage: tools csv_region_500w [VCF_f] [Distance] [min_SV_len]", complex_sv_region_detection_500W_run);
	ch.add_function("contig_file_split", "usage: tools contig_file_split [FC_Contig_L] [work_dir] [ref_fn]", contig_file_split_run);
	ch.add_function("FC_joint_ana", "usage: tools FC_joint_ana [contig.fa] [RepeatMasker.out] [TRF.anno] [TRUR/FALSE]", FC_joint_ana_run);

	ch.add_function("dump_ref_by_region", "usage: tools dump_ref_by_region <fasta_file> <region_file>", dump_ref_by_region_run);
	ch.add_function("get_ref_region", "usage: tools get_ref_region [ref.fa] [chr_ID:st_pos-ed_pos]", get_ref_region_run);
	ch.add_function("ref_split", "usage: tools ref_split [input.fa]", ref_split_run);
	ch.add_function("getReverseStr", "usage: tools getReverseStr [string (ACGT.....)]", getReverseStr_run);
	ch.add_function("entropy_analysis", "usage: tools entropy_analysis [bam_file] [ref.fa]", alignment_Entropy_Analysis_run);
	ch.add_function("small_var_den", "usage: tools small_var_den [bam_file] [ref.fa]", alignment_SMALL_VAR_DENSITY_Analysis_run);

	ch.add_function("align_seq", "tools align_seq", align_main_run);
	ch.add_function("genomic_contest_Analysis_SRS", "", genomic_contest_Analysis_SRS_run);
	ch.add_function("genomic_contest_Analysis_LRS", "", genomic_contest_Analysis_LRS_run);
	ch.add_function("genomic_contest_Analysis_ANNO", "", genomic_contest_Analysis_ANNO_run);
