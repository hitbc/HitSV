...
struct Alignment_Entropy_Analysis{
	...
	void entropy_analysis_help()
{
	fprintf(stderr, "Usage: entropy_analysis <region_bed> <ref.fa> <vcf_file> <bam_file> <min_sv_len>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Analyze alignment entropy for specified regions.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <region_bed>   : BED file with regions to analyze\n");
	fprintf(stderr, "  <ref.fa>       : Reference FASTA file\n");
	fprintf(stderr, "  <vcf_file>     : VCF file with structural variants\n");
	fprintf(stderr, "  <bam_file>     : BAM file with alignments\n");
	fprintf(stderr, "  <min_sv_len>   : Minimum SV length to consider\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: Entropy analysis results for each region\n");
}

int ana_run_Entropy(int argc, char *argv[]){
	if (argc < 6) {
		entropy_analysis_help();
		return 1;
	}
...
struct SMALL_VAR_DENSITY{
	...
	void small_var_den_help()
{
	fprintf(stderr, "Usage: small_var_den <region_bed> <ref.fa> <bam_file>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Analyze small variant density for specified regions.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <region_bed>   : BED file with regions to analyze\n");
	fprintf(stderr, "  <ref.fa>       : Reference FASTA file\n");
	fprintf(stderr, "  <bam_file>     : BAM file with alignments\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: Small variant density analysis results for each region\n");
}
	...
	char * LRS_bamFileName = argv[3];
		...
		for(uint r_i = 0; r_i < region_l.size(); r_i++){
			...
			//analysis P1:
			{
				...
				fprintf(fp_out, "R_FULL [%s:%d-%d]\t", faidx_iseq(c_ref_idx, r.chr_ID), r.st_pos, r.ed_pos);//regions
				...
				fprintf(fp_out, "I1=%f;T2=%f\t", gap_excluded_identity, gap_compressed_identity);//identity
				fprintf(fp_out, "AVE_MIS_BASE=%f;AVE_M_BASE=%f\t", AVE_MIS_BASE, AVE_M_BASE);//
				fprintf(fp_out, "R_LEN=%d;N_read=%d;N_SNP=%d;N_match_base=%d;N_INS=%d;N_DEL=%d;len_INS=%d;len_DEL=%d\n",
			...
			}
			{
				...
				fprintf(fp_out, "R_PART [%s:%d-%d]\t", faidx_iseq(c_ref_idx, r.chr_ID), r.st_pos, r.ed_pos);//regions
...
struct Ref_region_handler{
	void get_ref_region_help()
{
	fprintf(stderr, "Usage: get_ref_region <fasta_file> <region>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Extract reference sequence from FASTA file for specified region.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <fasta_file>   : Path to FASTA file (must have .fai index)\n");
	fprintf(stderr, "  <region>       : Region in format chr:start-end\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: Reference sequence for the specified region\n");
}
...
int get_ref_region(int argc, char *argv[]){  
  ...
  return 0;
	...
	static void ref_split_help()
{
	fprintf(stderr, "Usage: ref_split <fasta_file>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Split multi-sequence FASTA file into individual FASTA files.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <fasta_file>   : Path to multi-sequence FASTA file\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: Individual FASTA files named after each sequence\n");
}
...
static int ref_split(int argc, char *argv[])
{
	...
	void dump_ref_by_region_help()
{
	fprintf(stderr, "Usage: dump_ref_by_region <fasta_file> <region_file>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Extract reference sequences from FASTA file for specified regions.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <fasta_file>   : Path to FASTA file (must have .fai index)\n");
	fprintf(stderr, "  <region_file>  : Path to region file containing regions to extract\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: FASTA format sequences for each region\n");
}

int dump_ref_by_region(int argc, char *argv[])
{
	if (argc < 3) {
		dump_ref_by_region_help();
		return 1;
	}
	char *fasta_file = argv[1];
	...
	void getReverseStr_help()
{
	fprintf(stderr, "Usage: getReverseStr <sequence>\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Description: Get reverse complement of DNA sequence.\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Arguments:\n");
	fprintf(stderr, "  <sequence>     : DNA sequence to reverse complement\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Output: Reverse complement of the input sequence\n");
}
...
public:
	...
	// Check if sufficient arguments are provided
		if (argc < 4) {
			fprintf(stderr, "Error: Insufficient arguments for csv_region_500w\n");
			fprintf(stderr, "Usage: tools csv_region_500w [VCF_f] [Distance] [min_SV_len]\n");
			fprintf(stderr, "Example: tools csv_region_500w sample.vcf 500 50\n");
			fprintf(stderr, "\nParameters:\n");
			fprintf(stderr, "  VCF_f: Path to input VCF file containing structural variants\n");
			fprintf(stderr, "  Distance: Distance threshold (in bp) for defining complex SV regions\n");
			fprintf(stderr, "  min_SV_len: Minimum SV length (in bp) to consider\n");
			fprintf(stderr, "\nOutput: BED format regions with complex SV calls, including 500bp window regions\n");
			return 1;
		}
		char * vcf_fn_in = argv[1];//separate by ','
		CPX_distance = atoi(argv[2]);
	...
	void get_region_string(std::vector<std::string> &item_value, std::vector<std::string> & tmp, std::string & region_str, bool is_line_no_contig){
		region_str.clear();
		split_string(tmp, item_value[0].c_str() + 11, ":");
		region_str += tmp[0] + "_";
		std::string POS = tmp[1];
		split_string(tmp, POS.c_str(), "-");
		if(is_line_no_contig){
			int st_pos = atoi(tmp[0].c_str()) - 500;
			int end_pos = atoi(tmp[1].c_str()) + 500;
			region_str += std::to_string(st_pos) + "_" + std::to_string(end_pos);
		}
		else
			region_str += tmp[0] + "_" + tmp[1];
	}

	void get_region_for_loading_ref(std::vector<std::string> &item_value, std::vector<std::string> & tmp, std::string & region_str, bool is_line_no_contig){
		region_str.clear();
		split_string(tmp, item_value[0].c_str() + 11, ":");
		region_str += tmp[0] + ":";
		std::string POS = tmp[1];
		...
		if(is_line_no_contig){
			int st_pos = atoi(tmp[0].c_str()) - 500;
			int end_pos = atoi(tmp[1].c_str()) + 500;
			region_str += std::to_string(st_pos) + "-" + std::to_string(end_pos);
		}
		else
	...
	int contig_file_split(int argc, char *argv[]){
		// Check if sufficient arguments are provided
		if (argc < 4) {
			fprintf(stderr, "Error: Insufficient arguments for contig_file_split\n");
			fprintf(stderr, "Usage: tools contig_file_split [FC_Contig_L] [work_dir] [ref_fn]\n");
			fprintf(stderr, "Example: tools contig_file_split contig_list.txt ./work_dir reference.fa\n");
			fprintf(stderr, "\nParameters:\n");
			fprintf(stderr, "  FC_Contig_L: File containing list of contig files\n");
			fprintf(stderr, "  work_dir: Working directory for output files\n");
			fprintf(stderr, "  ref_fn: Reference FASTA file\n");
			return 1;
		}
		...
		for(uint sample_idx = 0; sample_idx < contig_FN_l.size(); sample_idx++ ){
			...
			for(uint c_idx = 0; c_idx < contigs_in_f.size(); c_idx++ ){
				...
				bool is_line_no_contig = (item_value.size() == 2);
				std::string region;
				get_region_string(item_value, item_value1, region, is_line_no_contig);
				//store:
				std::unordered_map<std::string , FILE*>::iterator it = region_index.find(region);
				//open file to store data and store ref
				FILE* store_data_f = NULL;
				if(it == region_index.end()){
					...
					get_region_for_loading_ref(item_value, item_value1, ref_region, is_line_no_contig);
					...
					store_data_f = try_open;
				}else{
					store_data_f = it->second;
				}
				...
				fwrite(&(store_string[0]), 1, store_string.size(), store_data_f);
			}
		}
		//close all files
