...
#define PRESET_ASM5 5
#define PRESET_ASM10 6
#define PRESET_ASM20 7
#define PRESET_M2 8
#define PRESET_SOMATIC 9
...
public:
	//as input
	void init(int preset_ID){
		km = km_init();
		memset(&ez, 0, sizeof(ksw_extz_t));
		//mapping options
		set_default_ksw_option(preset_ID);
		ksw_gen_mat_D();
		setRandomHeadTail();
	}
...
//fprintf(stderr, "S52 , km %ld, qlen  %ld, qseq %ld, ref_end_pos - ref_st_pos  %ld , tseq + ref_st_pos  %ld, ref_st_pos  %ld, 5  %ld, mata_D  %ld, gap_open_D  %ld, gap_ex_D  %ld, gap_open2_D  %ld, gap_ex2_D  %ld, bandwith  %ld, zdrop_D  %ld, -1  %ld, flag  %ld, &ez  %ld, \n",				   km, qlen, qseq, ref_end_pos - ref_st_pos, tseq + ref_st_pos, ref_st_pos, 5, mata_D, gap_open_D, gap_ex_D, gap_open2_D, gap_ex2_D, bandwith, zdrop_D, -1, flag, &ez);
	...
	void get_canditate_SVs(bool print_log, std::vector<NOVA_SV_FINAL_RST_item> &SVs,
		...
		for(int cigar_ID = 0;cigar_ID < n_cigar; cigar_ID++){
			...
			case 1:	//insertion
				...
				if(suggest_sv_len != 0 && c_length >= 1 && cigar_ID!=0 && cigar_ID!=n_cigar-1){
					maxSVlen = std::max(maxSVlen, c_length);
		...
		if(maxSVlen < minSVlen && SVs_size_ori != SVs.size()){
			SVs.erase(SVs.begin() + SVs_size_ori, SVs.end());
		}
		...
		//this code is removed, cause CPX will be showed as its original
		if(false){
	...
	void show_ref(){ fprintf(stderr, "show_ref: "); for(int i = 0; i < tlen; i++) fprintf(stderr, "%c", "ATCGNNN"[tseq[i]]); fprintf(stderr, "\n");}
	...
	bool has_ALU_INS(){
		...
		if(cigar_len == 0) {
			...
			for(int cigar_ID = 0; cigar_ID < cigar_len; cigar_ID++) {
				...
				if(c_type == 'I' && length >= 280 && length <= 350)
					return true;
			}
		}
	...
	int get_mapping_score(){
		return ez.score;
	}

	void print_mapping_score(){
		fprintf(stderr, "mapping_score: %d %d %d %d %d %d %d\n", ez.score, ez.reach_end, ez.max, ez.max_q, ez.max_t, ez.mqe, ez.mte);
	}
...
void set_option(int match_D, int mismatch_D, int gap_open_D, int gap_ex_D, int gap_open2_D, int gap_ex2_D, int zdrop_D, int bandwith, int ksw_flag){

		this->match_D = match_D;
		this->mismatch_D = mismatch_D;
		this->gap_open_D = gap_open_D;
		this->gap_ex_D = gap_ex_D;
		this->gap_open2_D = gap_open2_D;
		this->gap_ex2_D = gap_ex2_D;
		this->zdrop_D = zdrop_D;
		this->bandwith = bandwith;
		this->ksw_flag = ksw_flag;
	}
	void set_default_ksw_option(int preset_ID){
		if(preset_ID == PRESET_ASM5){
			set_option(1, 19, 39, 3, 81, 1, gap_open2_D + 600, 600, KSW_EZ_EQX);
		}
		else if(preset_ID == PRESET_ASM10){
			set_option(1, 9, 16, 2, 41, 1, gap_open2_D + 600, 600, KSW_EZ_EQX);
		}
		else if(preset_ID == PRESET_ASM20){
			set_option(1, 4, 6, 2, 26, 1, gap_open2_D + 600, 600, KSW_EZ_EQX);
		}
		else if(preset_ID == PRESET_M2){
			set_option(1, 15, 18, 3, 65, 1, gap_open2_D + 600, 600, KSW_EZ_EQX);
		}
		else if(preset_ID == PRESET_SOMATIC){
			set_option(2, 4, 4, 2, 24, 1, gap_open2_D + 600, 600, KSW_EZ_EQX);
		}
		else{
			fprintf(stderr,"ERROR: unknown preset_ID %d\n", preset_ID);
			exit(1);
		}
...
public:
	...
	int print_X_E_M2(FILE * output, int suggest_st_pos){
		log_output = output;
		int ref_pos_enough_match_base = 0;
		int contig_coverage = 0;
		int output_index = 0;
		int seq_i = 0;
		uint32_t* bam_cigar = ez.cigar;
		...
		for(int cigar_ID = 0;cigar_ID < cigar_len; cigar_ID++){
			...
			case 8://X
				for(int i = 0; i < cigar_len; i++, seq_i++)
				{
					if(qseq[seq_i] == tseq[output_index]){
						contig_coverage++;
						if(output_index < ref_pos_enough_match_base) fprintf(log_output, "M");//1 == M; 2 == X; 0 == -;
						else fprintf(log_output, "%c", "ACGTNNN"[qseq[seq_i]]);//1 == M; 2 == X; 0 == -;
					}
					else{
						fprintf(log_output, "%c", "acgtnnn"[qseq[seq_i]]);//1 == M; 2 == X; 0 == -;
					}
					output_index++;
				}	break;//M
			case 1:	
				//seq_i += cigar_len;	
				if(cigar_len > 10)
					fprintf(log_output, "<INS-%d-BG>%d", cigar_len, seq_i);  
				for(int i = 0; i < cigar_len; i++, seq_i++) {fprintf(log_output, "%c", "ACGTNNN"[qseq[seq_i]]); }
				if(cigar_len > 10) 
					fprintf(log_output, "%d<INS-%d-ED>", seq_i, cigar_len);  
			...
			case 2:	
				//D, print '-'
				if(cigar_len > 10)
					fprintf(log_output, "<DEL-%d-BG>%d", cigar_len, seq_i);  
				...
				if(cigar_len > 10) 
					fprintf(log_output, "%d<DEL-%d-ED>", seq_i, cigar_len);  
