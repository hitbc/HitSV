...
public:
	...
	void init(){
		...
		//mapping options
		Genotyping_read_aligner_copy_ksw_option();
		ksw_gen_mat_D();
	}
...
private:
	...
	void Genotyping_read_aligner_copy_ksw_option(){
			match_D = 2;
			mismatch_D= 6;
			gap_open_D= 24;
			gap_ex_D= 2;
...
struct NOVA_SV_TGS_INFO{
	...
	void setINFO(
		...
		xassert( this->not_supp_read_n >= 0, "");
		xassert( this->supp_read_n >= 0, "");
	...
	void setINFO_blank(){
		this->global_region_ID = -1;
		this->supp_read_n = -1;
		this->not_supp_read_n = -1;
		this->unknown_read_n = -1;
		this->haplotype_ID = -1;
		this->total_hap_number_in_region = -1;
		this->SV_in_HAP_ID = -1;
		this->total_SV_number_in_HAP = -1;
		this->SV_hap_position = -1;
	}
};

#define SOMATIC_BLANK -1
...
public:
	...
	void store_signal_region(int signal_region_st_, int signal_region_ed_){
		signal_region_st = signal_region_st_;
		signal_region_ed = signal_region_ed_;
	}
...
//	}
	void set_BND(const char * BND_type){
		this->is_BND = true;
		if (BND_type != nullptr) {
        	this->bnd_type = BND_type;
		} else {
			this->bnd_type = "";
		}
	}
	...
	bool REF_perfect_support_by_SRS(){
		//region_support_number[2][1] : not support contig
		//region_support_number[2][0] : support contig
		//region_support_number[2][2] : unknown
		return (SRS_read_perfect_number*1.2 > (region_support_number[2][1] + region_support_number[2][0] + region_support_number[2][2] ));
	}
	...
	bool writeVCF_final(kstring_t *s, bam_hdr_t * header, int *SV_global_ID){
		...
		//additional INFO:
		sprintf(s->s + s->l, ";SIGNAL_REGION=%s:%d-%d", header->target_name[chr_ID], signal_region_st, signal_region_ed); s->l += strlen(s->s + s->l);

		//output the contig when needed:
		if(!cigar.empty() && ABS(SV_length) >= 10){
			//CONTIG POSITION:
	...
	static void resultFilter(std::vector<NOVA_SV_FINAL_RST_item> & rst_l, int MIN_sv_len){
		...
		for(int i = 0; i < rst_num; i++)
			...
			if(rst_l[i].dis_SV_len_suggset_length <= smallest_SV_dis){
	...
	int get_ori_alignment_score(Genotyping_read_aligner * gra, bam1_t *br, int true_read_pos_bg,int region_ref_global_position, uint8_t * qseq_buff,
		...
		if(true){//running re-alignment
			...
			if(read_in_ref_st_pos < 0 || qlen < 0)
				return 0;
	...
	bool duplication_SV_filter(NOVA_SV_FINAL_RST_item & to_compare){
		...
		//mark VNTR-called result as duplication SV at first
		if(to_compare.isVNTRcallerRst == true)	to_compare.is_duplication_sv = true;
	...
	int long_deletion_DR_filter(Bam_file *bam_f, int normal_read_length, int max_isize){
		int withInR1_st_pos = st_pos - (max_isize - normal_read_length);
		if(withInR1_st_pos < 0) withInR1_st_pos = 0;
		RefRegion withInR1(chr_ID, withInR1_st_pos, st_pos);

		int withInR2_end_pos = endPos + (max_isize - normal_read_length - normal_read_length);
		if(withInR2_end_pos < 0) withInR2_end_pos = 0;
		RefRegion withInR2(chr_ID, endPos, withInR2_end_pos);

		int withInDel_end_pos = st_pos + (max_isize - normal_read_length - normal_read_length);
		if(withInDel_end_pos < 0) withInDel_end_pos = 0;
		RefRegion withInDel(chr_ID, st_pos, withInDel_end_pos);
	...
	int long_deletion_DR_filter_M2(Bam_file *bam_f, int normal_read_length, int max_isize){
		int withInR1_st_pos = st_pos - (max_isize - normal_read_length);
		if(withInR1_st_pos < 0) withInR1_st_pos = 0;
		RefRegion withInR1(chr_ID, withInR1_st_pos, st_pos);
		...
		bam_load_region.chr_ID = chr_ID;
		bam_load_region.st_pos = st_pos - max_isize;
		bam_load_region.ed_pos = endPos + max_isize;

		//Analyzer
		int support_read_number = 0;
		int NOT_support_read_number = 0;
		...
		while (bam_next(bam_f)) {
			...
			for (uint i = 0; i < br->core.n_cigar; ++i)
			{
				...
				case CIGAR_SEQ_MATCH:
				case CIGAR_DELETE:
					read_span += c_size; break;
				default:
					break;
				}
			}

			//fprintf(stderr, "ALL: Read name: %s; pos %d; mpos %d; isize %d\n", bam_qname(br), pos, mpos, isize);

			if(pos + read_span - 5 < st_pos && mpos > endPos){
				support_read_number++;
				//fprintf(stderr, "IN: Read name: %s; pos %d; mpos %d; isize %d\n", bam_qname(br), pos, mpos, isize);
			}else if(pos + read_span - 5 < st_pos && mpos > st_pos){
				NOT_support_read_number++;
				//fprintf(stderr, "OUT: Read name: %s; pos %d; mpos %d; isize %d\n", bam_qname(br), pos, mpos, isize);
			}

		}
		fprintf(stderr, "Deletion @ the DR filter, support_read_number %d, NOT_support_read_number %d\n", support_read_number, NOT_support_read_number);
		if(support_read_number *3 <= NOT_support_read_number){
	...
	void support_read_counting(bool print_log,  Bam_file *bam_f, Genotyping_read_aligner * gra,
		...
		for(int region_ID = 0; region_ID < 2; region_ID++){ //mode == 0 for region1, mode == 1 for region2
			...
			while (bam_next(bam_f)) {
				...
				int max_score = gra->getMaxScore(br->core.l_qseq - read_skip_left - read_skip_right);
				if(read_ori_score + 10 >= max_score) SRS_read_perfect_number++;	
		...
		fprintf(stderr,"SRS_read_perfect_number is %d\n", SRS_read_perfect_number);
	...
	int genotyping_LRS_Using_SRS(bool print_log, int normal_read_length, Bam_file *bam_f,
		...
		//fprintf(stderr, "st_pos is %d, endPos is %d\n", st_pos, endPos);

		int main_start = st_pos - 10;
		int supp_start = endPos - 10;

		if(main_start < 0) main_start = 0;
		if(supp_start < 0) supp_start = 0;
		
		RefRegion main(chr_ID, main_start, st_pos + edge_len);
		RefRegion supp(chr_ID, supp_start, endPos + edge_len);
	...
	void genotyping(int normal_read_length, int global_read_depth, int max_isize, Bam_file *bam_f,
		...
		//DR filter:
		deletion_pass_DR_filter = true;
		//show the DEL=DRP regions:
		if(final_genotype > 0 && isVNTRcallerRst && SV_length < -100 ){
			bool pass_dr_filter =  long_deletion_DR_filter_M2(bam_f, normal_read_length, max_isize);
			if( 0 == pass_dr_filter){
				...
				fprintf(stderr, "VNTR deletion PASS DR filter\n");
			}
		}else if(final_genotype > 0 && SV_length < -400){
	...
	static void write_to_vcf_header(FILE *vcf_output, bam_hdr_t * bam_header, std::string & command_with_time){
		//BASIC PART
		...
		fprintf(vcf_output, "##INFO=<ID=SIGNAL_REGION,Number=1,Type=String,Description=\"the signal region string\">\n");
		...
		fprintf(vcf_output, "##ALT=<ID=BND,Description=\"Breakpoint\">\n");
		...
		//COMMAND PART
		fprintf(vcf_output, "##%s\n", command_with_time.c_str());
		//PROGRAM VERSION
		fprintf(vcf_output, "##%s Version=%s\n", PACKAGE_NAME, PACKAGE_VERSION);
	...
	void combine_supp_read_TGS(NOVA_SV_FINAL_RST_item & b){
		...
		LRS_INFO.not_supp_read_n -= b.LRS_INFO.supp_read_n;
	...
	std::string get_contig_position_str(){
		if(contig_st_pos_in_region + region_ref_global_position == 837105)
			fprintf(stderr, " ");
		std::string contig_position_str = "CONTIG_POSITION=";
		contig_position_str += std::to_string(chr_ID) + ":" + std::to_string(contig_st_pos_in_region + region_ref_global_position)+ ";" ;

		contig_position_str += "CONTIG_HAPID=" + std::to_string(LRS_INFO.haplotype_ID)+ ";";
		return contig_position_str;
	}
...
private:
	...
	//signal results
	int signal_region_st;//the start position of the region used for collecting signals and calling SVs in the SV calling steps
	int signal_region_ed;//the end position of the region used for collecting signals and calling SVs in the SV calling steps
