...
bool BAM_handler::storeReadSR(bam1_t *br, int soft_left, int soft_right, int gap_mismatch_inside){
	...
	storeReadCore(Read_type::SR, br->core.l_qseq, storeReadBuff, bam_get_qual(br), br->core.n_cigar,
			bam_get_cigar(br), br->core.flag, br->core.pos, soft_left, soft_right, gap_mismatch_inside, br->core.qual);
	return true;
}

void BAM_handler::storeReadUM(bam1_t *br, uint8_t *query){
	storeReadCore(Read_type::UM, br->core.l_qseq, query, bam_get_qual(br), 0, 0, br->core.flag, br->core.pos, 0, 0, 0, br->core.qual);
}
...
//core function to store SRS reads
void BAM_handler::store_READ_END(int read_END){//check
	READ_LIST *rl =&read_list;
	rl->back().set_READ_END(read_END);
}

//core function to store SRS reads
void BAM_handler::store_LRS_ReadCore(int readLen, uint8_t *seq, uint8_t * qual, int cigarLen, uint32_t* bam_cigar,
		const char * bam_qname, int primary_chrID, int primary_pos, int flag,
		int32_t pos, int direction, int MAPQ){//check
	...
	rl->emplace_back();//new a read record
	rl->back().set(bam_qname, primary_chrID, primary_pos, flag, direction, pos, readLen, cigarLen, base.n, quality.n, cigar.n, 0, 0, 0, MAPQ);//new a read record
	//show data:
	if(false){
		//seq
		fprintf(stderr, "LRS_read@POS:%d readLen %d, cigarLen %d\n", pos, readLen, cigarLen);
		for(int i = 0; i < readLen; i++)
			fprintf(stderr, "%c", "ACGTNNNN"[base.a[base.n + i]]);
		fprintf(stderr, "\n");
		for(int i = 0; i < readLen; i++)
			fprintf(stderr, "%c", '!' + quality.a[quality.n + i]);
		fprintf(stderr, "\n");
		for(int i = 0; i < cigarLen; i++){
			fprintf(stderr, "%d%c",cigar.a[cigar.n + i].length, pathSTR[cigar.a[cigar.n + i].type]);
		}
		fprintf(stderr, "\n");
	}
...
void BAM_handler::storeReadCore(//check
		...
		int soft_left, int soft_right, int gap_mismatch_inside, int MAPQ){
	...
	if(t == Read_type::SR){
		...
		rl->emplace_back();
		rl->back().set(NULL, -1, -1, flag, direction, pos, readLen, cigarLen, base.n, quality.n, cigar.n, soft_left, soft_right, gap_mismatch_inside, MAPQ);//new a read record
		rl->back().read_end = -1;
	}
	else{
		READ_LIST *rl =&um_read_list;
		rl->emplace_back();
		rl->back().set(NULL, -1, -1, flag, 1 - mate_direction, pos, readLen, -1, base.n, quality.n, cigar.n, -1, -1, -1, MAPQ);//new a read record
		rl->back().read_end = -1;
	}
