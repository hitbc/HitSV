...
struct READ_record{
	...
	void set(const char * r_name, int primary_chrID, int primary_pos, uint16_t flag_, uint8_t direction_, uint64_t position_, int read_l_, int cigar_l_,
			uint64_t read_index_, uint64_t quality_index_, uint64_t cigar_index_,
			int soft_left_, int soft_right_, int NM_NUM_, int MAPQ_){
		this->r_name.clear();
		if(r_name != NULL)
			this->r_name.append(r_name);
		...
		read_l = read_l_;
		cigar_l = cigar_l_;
		//fprintf(stderr, "D1.1.2 SET: %ld\n", read_index_);
		xassert(read_index_ < 100000000000, "read_index_ is too large");
		read_index = read_index_;
		quality_index = quality_index_;
		cigar_index = cigar_index_;
		soft_left = soft_left_;
		soft_right = soft_right_;

		NM_NUM = NM_NUM_;
	}
	void set_READ_END(int read_end){
	...
	void show(){
		...
		fprintf(stderr, "[len: %d CIGAR_L: %d %ld]\t", read_l, cigar_l, read_index);
		fprintf(stderr, "[%ld %ld]\t", quality_index, cigar_index);
		fprintf(stderr, "[SL %d SR %d NM %d]\n", soft_left, soft_right, NM_NUM);
	}
};

namespace Read_type{
	enum T {SR, DR, UM, TL,LRS, unknown};
}
...
struct ASS_reads_info{
	...
	void set(const char *read_name, int read_in_ref_offset_, int read_st_pos_, int read_ed_pos_, int read_list_index_, int soft_left_, int soft_right_,
			int number_NM_, Read_type::T signal_type_){
		this->r_name.clear();
		if(read_name != NULL)
			this->r_name.append(read_name);
	...
	void set_LRS_info(int read_position, int read_end, int MAPQ, int flag, int primary_chrID, int primary_pos, bool LRS_full_begin, bool LRS_full_end){
		...
		this->primary_chrID = primary_chrID;
		this->primary_pos = primary_pos;
		this->LRS_full_begin = LRS_full_begin;
		this->LRS_full_end = LRS_full_end;
	}

	void set_LRS_info_zero(){
		set_LRS_info(0, 0, 0, 0, 0, 0, false, false);
	}

	bool is_clip_read(){
		return ((LRS_full_begin == 0 && soft_left != 0) || (LRS_full_end == 0 && soft_right != 0));
	}

	bool is_clip_left(){
		return ((LRS_full_begin == 0 && soft_left != 0));
	}

	bool is_clip_right(){
		return ((LRS_full_end == 0 && soft_right != 0));
	}

	int get_clip_mode(){
		if(is_clip_right())			return 1;
		else if(is_clip_left())		return 2;
		else						return 0;
	}

	void show_LRS_info(){
		std::cerr << "LRS info: " << r_name
			<< " read_position " << read_position
			<< " read_end " << read_end
			<< " MAPQ " << MAPQ
			<< " flag " << flag
			<< " primary_chrID " << primary_chrID
			<< " primary_pos " << primary_pos
			<< " LRS_full_begin " << LRS_full_begin
			<< " LRS_full_end " << LRS_full_end
			<< " soft_left " << soft_left
	...
	void print(FILE* log_file, int first_read_ID){
		...
		switch (signal_type) {
			...
			case Read_type::LRS: fprintf(log_file, "TGS"); break;
			default:  fprintf(log_file, "unknown"); break;
		}
		fprintf(log_file, " id: %d(%d) {%d~%d}, ref pos %d, {%d, %d, %d}]\t",
				read_list_index - first_read_ID, read_list_index, read_st_pos, 
				read_ed_pos, read_in_ref_offset, soft_left, soft_right, number_NM);
	}
};
...
#define MIN_BASE_QUAL 6
	void add_read_bin(Read_type::T t, uint8_t * read_str, uint8_t * qual_str, int read_len, int read_in_ref_offset_, int left_clip, int right_clip, int NM_num, int read_list_index_){
		...
		ass_read_list.emplace_back();
		ass_read_list.back().set(NULL, read_in_ref_offset_, read_st_pos, read_ed_pos, read_list_index_, left_clip, right_clip, NM_num, t);
		ass_read_list.back().set_LRS_info_zero();
...
//	        last_N_base = i;
	...
	void add_read_TL(char * read_str, uint8_t * qual_str, int read_len, bool store_in_reverse, int read_list_index_, int tid, int pos){
		...
		ass_read_list.emplace_back();
		ass_read_list.back().set(NULL, -1, tid, pos, read_list_index_, -1, -1, store_in_reverse, Read_type::TL);
		ass_read_list.back().set_LRS_info_zero();
		reads.emplace_back(read_str);
	}

	//adding LRS reads into assembly read list
	void add_read_LRS(const char *read_name, std::string & read_str, int read_list_index_, int tid, int pos,
			bool is_full_begin, bool is_full_end, int soft_left, int soft_right, 	
			int read_position, int read_end, int MAPQ, int flag, int primary_chrID, int primary_pos){
		ass_read_list.emplace_back();
		ass_read_list.back().set(read_name, -1, tid, pos, read_list_index_, soft_left, soft_right, 0, Read_type::LRS);

		ass_read_list.back().set_LRS_info(read_position, read_end, MAPQ, flag, tid, primary_pos, is_full_begin, is_full_end);
		reads.emplace_back(read_str);
	}
	...
	void run_assembly(MainAssemblyHandler *am){
		...
		for(uint i = 0; i < read_size; i++){
			if(ass_read_list[i].signal_type == Read_type::LRS){
...
public:
	...
	void init(const char * bamFileName, char *realignment_bamFileName, char *TL_read_Filename, char * ref_file_name){
		const int MAX_READ_INDEX_SIZE = SEGMENT_LEN/100 + 1;
		sr_read_index = new int64_t [MAX_READ_INDEX_SIZE];
		um_read_index = new int64_t [MAX_READ_INDEX_SIZE];
		storeReadBuff = new uint8_t[300000000];//300MBP at MAX
		kv_init(base);
		kv_init(quality);
		kv_init(cigar);
	...
	void store_LRS_ReadCore(int readLen, uint8_t *seq, uint8_t * qual, int cigarLen, uint32_t* bam_cigar,
		 const char * bam_qname, int primary_chrID, int primary_pos, int flag, int32_t pos, int direction, int MAPQ);
	void store_READ_END(int read_END);
	void storeReadCore(	Read_type::T t, int readLen, uint8_t *seq, uint8_t * qual,	int cigarLen,
			uint32_t* bam_cigar, uint16_t flag, int32_t pos,	int soft_left, int soft_right, int gap_mismatch_inside, int MAPQ);
	...
	//read_pos_at_ref_pos_st
	void load_read_LRS_ASM(std::string &s_store, READ_record& c_r,
			int region_to_load_st_in_ref, int region_to_load_ed_in_ref,
			int &corresponding_read_position_load_st, int &corresponding_read_position_load_ed,
			int &read_mapping_pos_in_ref, int &read_mapping_end_in_ref,
			bool &is_full_begin, bool &is_full_end){
		s_store.clear();
		//skip no-relation-reads when searching all the reads list to Accelerate the searching
		if(c_r.position > region_to_load_ed_in_ref || c_r.read_end < region_to_load_st_in_ref){ return; }
		bool using_clip = true;
		int MIN_QUAL_TGS = 0;
		//store ref position to read position:
		is_full_begin = true;
		is_full_end = true;
		corresponding_read_position_load_st = -1;
		corresponding_read_position_load_ed = -1;
		int read_position = 0;

		if(c_r.position > region_to_load_st_in_ref )
			is_full_begin = false;
		//
		...
		for (uint i = 0; i < n_cigar; ++i)
		{
			...
			case CIGAR_MATCH: case CIGAR_SEQ_MATCH: case CIGAR_SEQ_MISMATCH:
				for(int i = 0; i < c_size; i++, seq_i++, ref_i++){
					if(ref_i >= region_to_load_st_in_ref && ref_i < region_to_load_ed_in_ref){
						if(ref_i == region_to_load_st_in_ref){
							corresponding_read_position_load_st = read_position + i;
						}
						if(ref_i == region_to_load_ed_in_ref - 1){
							corresponding_read_position_load_ed = read_position + i;
						}
						uint8_t c = "ACGTNN"[read_str[seq_i]];
						if(qual_str[seq_i] < MIN_QUAL_TGS) c = 'N';
						s_store.push_back(c);
					}
				}
				read_position += c_size;
				break;
			case CIGAR_INSERT:
				for(int i = 0; i < c_size; i++, seq_i++){
					...
					if(ref_i == region_to_load_st_in_ref){ 	corresponding_read_position_load_st = read_position + i;	}
					if(ref_i == region_to_load_ed_in_ref - 1){corresponding_read_position_load_ed = read_position + i;	}
				}
				read_position += c_size;
				break; //do nothing
			case CIGAR_DELETE:
				for(int i = 0; i < c_size; i++, ref_i++);
				if(region_to_load_st_in_ref >= ref_i - c_size && region_to_load_st_in_ref < ref_i){ 	corresponding_read_position_load_st = read_position;	}
				if(region_to_load_ed_in_ref >= ref_i - c_size && region_to_load_ed_in_ref < ref_i){ 	corresponding_read_position_load_ed = read_position;	}
				break;
			case CIGAR_SOFT_CLIP:
				//clip out of range in the begin
				if(ref_i < region_to_load_st_in_ref || ref_i > region_to_load_ed_in_ref)	using_clip = false;
				else											using_clip = true;

				if(using_clip && c_size < 100000){
					for(int i = 0; i < c_size; i++, seq_i++)
					{
						uint8_t c = "ACGTNN"[read_str[seq_i]];
						if(read_str[seq_i] < MIN_QUAL_TGS) c = 'N';
						s_store.push_back(c);
					}
				}else{
					seq_i += c_size;
		...
		read_mapping_end_in_ref = ref_i;
		if(ref_i < region_to_load_ed_in_ref)
			is_full_end = false;
	}

	void load_read_LRS(std::string &s_store, READ_record& c_r,
			int ref_pos_st, int ref_pos_ed,
			bool &is_full_begin, bool &is_full_end){
		s_store.clear();
		//skip no-relation-reads when searching all the reads list to Accelerate the searching
		if(c_r.position > ref_pos_ed || c_r.read_end < ref_pos_st){ return; }
		...
		uint8_t * qual_str = getQualStr(c_r);
		//fprintf(stderr, "D1.1\n");
		//fprintf(stderr, "D1.1.1 %d %ld %d %ld \n", c_r.position, c_r.read_index, ref_pos_ed, read_str);
		uint32_t n_cigar = c_r.cigar_l;
		int seq_i = 0;
		int ref_i = c_r.position;
		for (uint i = 0; i < n_cigar; ++i)
		{
			...
			int c_type = p[i].type;
			int c_size = p[i].length;
			//fprintf(stderr, "D1.2 %d %d %d\n",c_type,c_size,n_cigar);
			switch (c_type){
			case CIGAR_MATCH: case CIGAR_SEQ_MATCH: case CIGAR_SEQ_MISMATCH:
				for(int i = 0; i < c_size; i++, seq_i++, ref_i++){
					if(ref_i >= ref_pos_st && ref_i < ref_pos_ed){
						uint8_t c = "ACGTNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN"[read_str[seq_i]];
						if(qual_str[seq_i] < MIN_QUAL_TGS) c = 'N';
						s_store.push_back(c);
						//if(c != 'A' && c != 'C' && c != 'G' && c != 'T')	xassert(false, "");
					}
				}
				break;
			case CIGAR_INSERT:
			...
			case CIGAR_SOFT_CLIP:
				//clip out of range in the begin
				if(ref_i < ref_pos_st || ref_i > ref_pos_ed)	using_clip = false;
				else											using_clip = true;
				if(using_clip && c_size < 100000){
					for(int i = 0; i < c_size; i++, seq_i++)
					{
						uint8_t c = "ACGTNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN"[read_str[seq_i]];
						//if(i < 200) fprintf(stderr, "D1.3 %c %d %d %d\n", c, qual_str[seq_i],i,seq_i);
						//if(read_str[seq_i] > 4) fprintf(stderr, "D1.4 %d %d\n", read_str[seq_i], qual_str[seq_i]);
						if(qual_str[seq_i] < MIN_QUAL_TGS) c = 'N';
						s_store.push_back(c);
						//if(c != 'A' && c != 'C' && c != 'G' && c != 'T')	xassert(false, "");
					}
		...
		if(false){
			for(int i = 0; i < s_store.size(); i++){
				if(s_store[i] != 'A' && s_store[i] != 'C' && s_store[i] != 'G' && s_store[i] != 'T')
					xassert(false, "");
			}
		}
	...
	void build_read_index_SINGLE(READ_LIST &l, int64_t *index, int region_st_index){
		for(int64_t i = 0; i < READ_INDEX_SIZE; i++)
			index[i] = -1;
		int64_t read_ID = 0;
		for(auto r: l){
			int64_t c_index = (r.position - region_st_index)/ 100;
	...
	int search_reads(Read_type::T t, Ass_Block & ab, int st_pos, int ed_pos, int max_load, int region_st_index){
		...
		READ_LIST *c_read_l = NULL; int64_t *index = NULL;
		...
		int64_t c_index = (st_pos - region_st_index) / 100;
		...
		int64_t search_st = index[c_index];
		int64_t list_size = l.size();
...
private:
	...
	int64_t * sr_read_index;
	int64_t * um_read_index;
