...
struct RefLocalRepeat{
	...
	void setDefaultRegionStepSize(int region_step_size_){
		region_step_size = region_step_size_;
	}
...
private:
	...
	struct BAM_header_ITEM
	{
		std::string target_name;
		uint32_t target_len;
		BAM_header_ITEM(const char * target_name_, uint32_t target_len_){
			target_name = std::string(target_name_);
			target_len = target_len_;
		}
	};

	std::vector<BAM_header_ITEM> bam_header_items;
...
public:
	...
	void init(
		...
		cur_r.set_no_check(st_chr_ID, st_pos - SEGMENT_LEN, st_pos);
		curRefRegion.set_init();
		c_ref_idx = NULL;
	...
	void registHeader(bam_hdr_t* bam_header_){
		if (bam_header_items.size() == 0)
		{
			for(int i = 0; i < bam_header_->n_targets; i++){
				bam_header_items.emplace_back(bam_header_->target_name[i], bam_header_->target_len[i]);
			}
		}
	}
	...
	char * load_ref_by_region(int tid, int st_pos, int ed_pos, int * max_load){
		...
		if(tid >= bam_header_items.size())
			return NULL;
		sprintf(ref_region_char, "%s:%d-%d", bam_header_items[tid].target_name.c_str(), st_pos, ed_pos);
		return fai_fetch(c_ref_idx, ref_region_char, max_load);
	}

	bool load_seg_index(){
		//get new current handle region:
		if(cur_r.st_pos + SEGMENT_LEN > (int)bam_header_items[cur_r.chr_ID].target_len - 1)
		...
		if(cur_r.chr_ID < bam_header_items.size())
			cur_r.ed_pos = MIN(cur_r.ed_pos, (int)bam_header_items[cur_r.chr_ID].target_len - 1);
		if(cur_r.chr_ID == ed_chr_ID)
			cur_r.ed_pos = MIN(cur_r.ed_pos, ed_pos);

		//when reaching end of
		if(cur_r.chr_ID >= bam_header_items.size() || //reach end of all target
...
private:
	...
	bool load_reference(int chr_ID){
		...
		if(chr_ID >= bam_header_items.size()) return false;

		int load_ref_length = 0;
		xassert(bam_header_items.size() != 0 && chr_ID < bam_header_items.size(), "BAM header should be registered for a REF_BUFF before loading reference!\n");
		if(c_ref_idx == NULL)//load index (build new index when without index file), open reference file
			c_ref_idx = reference_index_load(referenceFilename);
		char * c_reference = get_reference_region(c_ref_idx, bam_header_items[chr_ID].target_name.c_str(), &load_ref_length);
