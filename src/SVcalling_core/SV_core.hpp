...
struct SIGNAL_PARAMETER{
	...
	void reset_depth(float average_read_depth){
		read_depth = average_read_depth;
		SVE_MIN_SOLID_SCORE = average_read_depth * 0.08;// 40X ----> 3; 60X -----> 4; 29X -------> 2;
		SVE_MIN_READ_NUM = average_read_depth * 0.1;//40X ---> 4; 60X ------> 6; 29X ------> 2;
		SVE_MIN_SOLID_SCORE = MAX(2, SVE_MIN_SOLID_SCORE);//at lease 2
		SVE_MIN_READ_NUM = MAX(3, SVE_MIN_READ_NUM);//at least 3

		SVE_combine_min_score_step1 = average_read_depth * 0.4; //66-> 26; 44 -> 17; 30 -> 12
...
struct LRS_SIG{
	...
	bool region_overlap(LRS_SIG &to_compare, int overlap_edge){
		int A_bg = POS - overlap_edge;
		int B_bg = to_compare.POS - overlap_edge;
		if (A_bg < 0)  A_bg = 0;
		if (B_bg < 0)  B_bg = 0;
		RefRegion A(chrID, A_bg, END + overlap_edge);
		RefRegion B(chrID, B_bg, to_compare.END + overlap_edge);
	...
	void show_simple(){
		...
		std::string type_str;
		switch(type){
			case LRS_SIG_INS: type_str = "LRS_SIG_INS"; break;
			case LRS_SIG_DEL: type_str = "LRS_SIG_DEL"; break;
			case LRS_SIG_INV: type_str = "LRS_SIG_INV"; break;
			case LRS_SIG_TRA: type_str = "LRS_SIG_TRA"; break;
			case LRS_SIG_CLIP_LEFT: type_str = "LRS_SIG_CLIP_LEFT"; break;
			case LRS_SIG_CLIP_RIGHT: type_str = "LRS_SIG_CLIP_RIGHT"; break;
			case LRS_SIG_X: type_str = "LRS_SIG_X"; break;
			default: type_str = "UNKNOWN"; break;
		}	

		fprintf(stderr, "LRS SIG: \t");
		fprintf(stderr, "[%d:%ld-%ld];[type %s; isUsed:%d, support_read_Num:%d]\n", chrID, POS, END, type_str.c_str(), isUsed, support_read_Num);
	}
...
struct LRS_SA_INFO{
	...
	int sa_in_ref_position_bg = 0;
	int sa_in_ref_position_ed = 0;
	...
	void store(int sa_idx, std::vector<std::string> &item_tmp, faidx_t * c_ref_idx, int total_read_len){
		...
		int ref_span = calculate_ref_span_from_cigar(sa_cigar_l);
		sa_in_ref_position_bg = SA_POS;
		sa_in_ref_position_ed = SA_POS + ref_span;
	}

	void store_main(int SA_chrID, int SA_POS, path_segment *main_cigar_st, path_segment *main_cigar_ed, int total_read_len, bool mapping_direction){
		//chr5,47154842,+,87624S77370M374I1749543S,1,12794
		this->SA_chrID = SA_chrID;
		this->SA_POS = SA_POS;
		sa_direction = mapping_direction;
	...
	inline int64_t calculate_ref_span_from_cigar(const std::vector<path_segment>& cigar_l) {
		int64_t ref_span = 0;
		for (const auto& seg : cigar_l) {
	...
	void show(){
		fprintf(stderr,
				...
				"sa_in_read_position: [%d-%d, len %d,diff %d], sa_in_ref_position: [%d-%d, len %d], sa_direction %d"
				"\n"
				,
				sa_idx, SA_chrID, SA_POS, SA_MAPQ, SA_number_of_mismatch,

				sa_in_read_position_bg, sa_in_read_position_ed,
				sa_in_read_position_ed - sa_in_read_position_bg,
				SA_POS - sa_in_read_position_bg,

				sa_in_ref_position_bg, sa_in_ref_position_ed,
				sa_in_ref_position_ed - sa_in_ref_position_bg,

				sa_direction );

		//show CIGAR:
		for(uint i = 0; i < sa_cigar_l.size(); i++){
			fprintf(stderr, "%d%c", sa_cigar_l[i].length, pathSTR[sa_cigar_l[i].type]);
		}
		fprintf(stderr, "\n");
	}
	...
	void show_2(){
		fprintf(stderr,
				"REF: %d:%d-%d;Dir:%d;is_main:%d;READ:%d-%d\t", 
				SA_chrID, sa_in_ref_position_bg, sa_in_ref_position_ed, sa_direction, is_main, sa_in_read_position_bg, sa_in_read_position_ed);
	}

    static inline int cmp_by_main_and_read_pos(const LRS_SA_INFO &a, const LRS_SA_INFO &b){
        //var basic
...
struct NearestSAInfo {
	int sa_idx = -1;       // nearest position SA's idx
	bool is_bg = true;     // true=bg, false=ed
	int distance = 0;      // distance to pos
	LRS_SA_INFO sa;
	...
	void show(){
		fprintf(stderr, "NearestSAInfo: distance %d, is_bg %d\t", distance, is_bg);
		sa.show_2();
		fprintf(stderr, "\n");
	}
};
...
struct LRS_SA_IN_READ{
	...
	int chrID;
	int pos;
	bool is_primary;//bam record is PRIMARY alignment
	int total_read_len;
	std::vector<LRS_SA_INFO> sa_l;
	LRS_SA_INFO sa_main;

	NearestSAInfo nearest_sa_read_position;

	void store_basic(int read_ID, int chrID, int64_t pos, int bam_is_primary, int total_read_len, path_segment *main_cigar_st, path_segment *main_cigar_ed, bool mapping_direction){
		...
		sa_l.clear();
		sa_l.emplace_back();
		sa_l.back().store_main(chrID, pos, main_cigar_st, main_cigar_ed, total_read_len, mapping_direction);
		sa_main = sa_l.back();
	}
	...
	//sort
	void sort_by_primary_and_read_position(){
		std::sort(sa_l.begin(), sa_l.end(), LRS_SA_INFO::cmp_by_main_and_read_pos);
	}
	...
	void show_2(){
		fprintf(stderr, "read_ID %d,is_primary %d total_read_len %d\t", read_ID, is_primary, total_read_len);
		for(uint i = 0; i < sa_l.size(); i++){
			LRS_SA_INFO & sa = sa_l[i];
			sa.show_2();
		}
	...
	void show_nearest_sa_read_position(){
		nearest_sa_read_position.show();
	}

	void find_nearest_sa_read_position(int pos, int main_mapping_direction)
	{
		nearest_sa_read_position.clear();
		if (sa_l.empty()) return;

		int min_dist = INT_MAX;

		for (int i = 0; i < sa_l.size(); i++) {
			const LRS_SA_INFO& sa = sa_l[i];
			//skip main SA
			if(sa.is_main)	continue;

			bool same_direction = (sa.sa_direction == main_mapping_direction);
			int cmp_pos = pos;
			if(!same_direction)	cmp_pos = total_read_len - pos;

			// check bg
			int dist_bg = abs(sa.sa_in_read_position_bg - cmp_pos);
			if (dist_bg < min_dist) {
				min_dist = dist_bg;
				nearest_sa_read_position.sa_idx = i;
				nearest_sa_read_position.is_bg = true;
				nearest_sa_read_position.distance = dist_bg;
			}

			// check ed
			int dist_ed = abs(sa.sa_in_read_position_ed - cmp_pos);
			if (dist_ed < min_dist) {
				min_dist = dist_ed;
				nearest_sa_read_position.sa_idx = i;
				nearest_sa_read_position.is_bg = false;
				nearest_sa_read_position.distance = dist_ed;
			}
		}
		...
		if(nearest_sa_read_position.sa_idx != -1)
			nearest_sa_read_position.sa = sa_l[nearest_sa_read_position.sa_idx];
	}
...
struct LRS_SA_Handler{
	...
	std::unordered_map<std::string, LRS_SA_IN_READ> sa_read_l;

	void clear(){
		sa_read_l.clear();
	}

	void store(const char * qname, int read_ID, int chrID, int64_t pos, bool bam_is_primary,
			path_segment *main_cigar_st, path_segment *main_cigar_ed,
			const char * SA_string, int total_read_len, faidx_t * c_ref_idx, bool mapping_direction){
		//fprintf(stderr, "SA_string %s \n", SA_string);
		split_string(item_tmp, SA_string, ";");
		std::string qname_str(qname);
		qname_str += "_" + std::to_string(pos);

		//discarded code
		if(false){
			//already with data, find a new key for the read
			if(sa_read_l.find(qname_str) != sa_read_l.end()){
				for(int i = 1; i < MAX_int32t; i++){
					std::string qname_str_ = qname_str + "_" + std::to_string(i);
					if(sa_read_l.find(qname_str_) == sa_read_l.end()){
						qname_str = qname_str_;
						break; 
					}
				}
			}
		...
		sa_read_l[qname_str] = LRS_SA_IN_READ();
		sa_read_l[qname_str].store_basic(read_ID, chrID, pos, bam_is_primary, total_read_len, main_cigar_st, main_cigar_ed, mapping_direction);
		sa_read_l[qname_str].store_SA_full(item_tmp, c_ref_idx);
		//sort by the position of reads
		sa_read_l[qname_str].sort_by_primary_and_read_position();
		//show all data
		if(false) sa_read_l[qname_str].show();
	}
};
...
struct SUPPORT_READ_LIST_ITEM{
	...
	int read_mapping_pos;

	SUPPORT_READ_LIST_ITEM(int read_id, std::string read_name, int read_mapping_pos){
		this->read_id = read_id;
		share_target_num = 1;
		is_best_target = true;
		this->read_name = read_name;
		this->read_mapping_pos = read_mapping_pos;
	}
...
struct TARGET_path_and_index{
	...
	//
	std::string contig;
	std::string contig_string_quality;
	...
	void get_the_contig_string(bool print_log, std::vector<UNITIG_INFO_item> &unitig_l,
			std::vector<std::string> & read_string, bool is_skip_depth_1_kmer);
	void get_the_contig_string_somatic_mode(bool print_log, std::vector<UNITIG_INFO_item> &unitig_l,
		std::vector<std::string> & read_string, bool is_skip_depth_1_kmer);
	void show_the_final_result(bool print_log, int cur_target_ID, std::vector<UNITIG_INFO_item> &unitig_l, std::vector<ASS_reads_info> &ass_read_list);
	void show_read_list(const char * title_str, std::vector<ASS_reads_info> &ass_read_list, int haplotype_ID);
	void show_the_target_path();
	void show_total_length(std::vector<UNITIG_INFO_item> &unitig_l);
	void build(std::vector<READ_PATH_COMBINE_ITEM> & target_seq, int read_id, std::string &read_name, int read_mapping_pos, std::vector<UNITIG_INFO_item> & unitig_l,
	...
	bool SDP_update_and_branch_condition_check(
			...
			std::vector<READ_PATH_COMBINE_ITEM> &query, int read_id, std::string &read_name, int read_mapping_pos, bool LRS_full_begin, bool LRS_full_end, bool is_clip_read, //read info
...
struct CONTIG_SELECT{
	...
	void set(int support_read_number, int contig_ID,
			...
			int not_support_read_number, int contig_cmp_len, int contig_len, float avg_mapq){
...
struct CONTIG_SELECTER{
	...
	void contig_basic_count(bool print_log, std::vector<TARGET_path_and_index> &target_path_idx_l, std::vector<READ_MATCH_NUM> & rml, std::vector<ASS_reads_info> &ass_read_list);
...
struct LRS_Somatic_BND_Handler
{
	//result handler
	struct BND_signal{
		std::string read_name;
		int main_BP_read_position;
		int main_BP_ref_position;
		int main_BP_ref_chrID;
		int nearest_BP_ref_chrID;
		int nearest_BP_ref_position;
		bool same_direction;

		bool main_BP_is_left;
		bool nearest_BP_is_left;

		void store(std::string &read_name, int main_BP_read_position, int main_BP_ref_position, int main_BP_ref_chrID, int nearest_BP_ref_chrID, int nearest_BP_ref_position, bool same_direction, bool main_BP_is_left, bool nearest_BP_is_left){
			this->read_name = read_name;
			this->main_BP_read_position = main_BP_read_position;
			this->main_BP_ref_position = main_BP_ref_position;
			this->main_BP_ref_chrID = main_BP_ref_chrID;
			this->nearest_BP_ref_chrID = nearest_BP_ref_chrID;
			this->nearest_BP_ref_position = nearest_BP_ref_position;
			this->same_direction = same_direction;
			this->main_BP_is_left = main_BP_is_left;
			this->nearest_BP_is_left = nearest_BP_is_left;
		}

		void show(){
			fprintf(stderr, "read_name %s\tmain_BP_read_position %d\tmain_BP_ref_position %d\tmain_BP_ref_chrID %d\tnearest_BP_ref_chrID %d\tnearest_BP_ref_position %d\tsame_direction %d \tmain_BP_is_left %d\tnearest_BP_is_left %d\n",
					read_name.c_str(), main_BP_read_position, main_BP_ref_position, main_BP_ref_chrID, nearest_BP_ref_chrID, nearest_BP_ref_position, same_direction, main_BP_is_left, nearest_BP_is_left);
		}
	};

	// cluster result: each cluster is a group of indices in bnd_sig_list
	struct BNDClusterResult {
		std::vector<std::vector<size_t>> clusters;  // each cluster is a group of indices in bnd_sig_list
	};

	//public:
	std::vector<BND_signal> bnd_sig_list;
	BNDClusterResult result;
	BND_signal BND_max_cluster;
	int max_idx; // index of the cluster with the maximum size
	int max_size;// maximum size of the cluster

	void init(){ 
		bnd_sig_list.clear(); 
	}

	void cluster_bnd_signal_list( bool log_output = true)
	{
		result.clusters.clear();

		if (bnd_sig_list.empty()) return;

		const int POS_TOLERANCE = 20;

		// ====== Step 1: group by 5 exact fields ======
		// key = main_BP_is_left + nearest_BP_is_left + same_direction + main_BP_ref_chrID + nearest_BP_ref_chrID
		std::map<std::string, std::vector<size_t>> groups;
		for (size_t i = 0; i < bnd_sig_list.size(); ++i) {
			const auto& b = bnd_sig_list[i];
			char key[256];
			snprintf(key, sizeof(key), "%d_%d_%d_%d_%d",
				(int)b.main_BP_is_left,
				(int)b.nearest_BP_is_left,
				(int)b.same_direction,
				b.main_BP_ref_chrID,
				b.nearest_BP_ref_chrID);
			groups[key].push_back(i);
		}

		// ====== Step 2: cluster within each group by position (20bp tolerance) ======
		for (auto& [key, idx_vec] : groups) {
			// sort by main_BP_ref_position
			std::sort(idx_vec.begin(), idx_vec.end(), [&](size_t a, size_t b) {
				if (bnd_sig_list[a].main_BP_ref_position != bnd_sig_list[b].main_BP_ref_position)
					return bnd_sig_list[a].main_BP_ref_position < bnd_sig_list[b].main_BP_ref_position;
				return bnd_sig_list[a].nearest_BP_ref_position < bnd_sig_list[b].nearest_BP_ref_position;
			});

			std::vector<bool> used(idx_vec.size(), false);
			for (size_t i = 0; i < idx_vec.size(); ++i) {
				if (used[i]) continue;
				used[i] = true;
				std::vector<size_t> cluster;
				cluster.push_back(idx_vec[i]);

				const auto& center = bnd_sig_list[idx_vec[i]];
				int main_min = center.main_BP_ref_position;
				int main_max = center.main_BP_ref_position;
				int near_min = center.nearest_BP_ref_position;
				int near_max = center.nearest_BP_ref_position;

				// expand cluster until no more elements can be added
				bool changed = true;
				while (changed) {
					changed = false;
					for (size_t j = 0; j < idx_vec.size(); ++j) {
						if (used[j]) continue;
						const auto& cand = bnd_sig_list[idx_vec[j]];
						if (cand.main_BP_ref_position >= main_min - POS_TOLERANCE &&
							cand.main_BP_ref_position <= main_max + POS_TOLERANCE &&
							cand.nearest_BP_ref_position >= near_min - POS_TOLERANCE &&
							cand.nearest_BP_ref_position <= near_max + POS_TOLERANCE) {
							used[j] = true;
							cluster.push_back(idx_vec[j]);
							main_min = std::min(main_min, cand.main_BP_ref_position);
							main_max = std::max(main_max, cand.main_BP_ref_position);
							near_min = std::min(near_min, cand.nearest_BP_ref_position);
							near_max = std::max(near_max, cand.nearest_BP_ref_position);
							changed = true;
						}
					}
				}
	...
	// ==================== New function: compute max cluster and output to BND_max_cluster ====================
	void find_max_cluster() {
		...
		for (size_t i = 0; i < result.clusters.size(); ++i) {
			if ((int)result.clusters[i].size() > max_size) {
				max_size = result.clusters[i].size();
				max_idx = i;
			}
		}

		const auto& max_cluster = result.clusters[max_idx];
		if (max_cluster.empty()) return;

		// The 5 exact-match fields are identical within the same cluster, take the first
		const auto& first = bnd_sig_list[max_cluster[0]];
		BND_max_cluster.main_BP_is_left      = first.main_BP_is_left;
		BND_max_cluster.nearest_BP_is_left   = first.nearest_BP_is_left;
		BND_max_cluster.same_direction       = first.same_direction;
		BND_max_cluster.main_BP_ref_chrID    = first.main_BP_ref_chrID;
		BND_max_cluster.nearest_BP_ref_chrID = first.nearest_BP_ref_chrID;

		// main_BP_ref_position: take the mode
		std::map<int, int> main_freq;
		for (size_t idx : max_cluster) {
			main_freq[bnd_sig_list[idx].main_BP_ref_position]++;
		}
		int main_mode = 0, main_mode_cnt = 0;
		for (const auto& [pos, cnt] : main_freq) {
			if (cnt > main_mode_cnt) {
				main_mode_cnt = cnt;
				main_mode = pos;
			}
		}
		BND_max_cluster.main_BP_ref_position = main_mode;

		// nearest_BP_ref_position: take the mode
		std::map<int, int> near_freq;
		for (size_t idx : max_cluster) {
			near_freq[bnd_sig_list[idx].nearest_BP_ref_position]++;
		}
		int near_mode = 0, near_mode_cnt = 0;
		for (const auto& [pos, cnt] : near_freq) {
			if (cnt > near_mode_cnt) {
				near_mode_cnt = cnt;
				near_mode = pos;
			}
		}
		BND_max_cluster.nearest_BP_ref_position = near_mode;

		// Summary marker
		BND_max_cluster.read_name = "MAX_CLUSTER";
		BND_max_cluster.main_BP_read_position = 0;

		// LOG output
		fprintf(stderr, "========== MAX cluster (size: %zu/%zu) ==========\n",
			max_size, bnd_sig_list.size());
		BND_max_cluster.show();
		fprintf(stderr, "  main_BP_ref_position mode: %d (freq: %d/%zu)\n",
			main_mode, main_mode_cnt, max_cluster.size());
		fprintf(stderr, "  nearest_BP_ref_position mode: %d (freq: %d/%zu)\n",
			near_mode, near_mode_cnt, max_cluster.size());
		fprintf(stderr, "==================================================\n");
	}

	void show_max_cluster(){
		
		BND_max_cluster.show();
	}
	
	void show_bnd_signal_list(){
			//show the BND signal list:
		fprintf(stderr, "BND signal list:\n");
		for(BND_signal & n : bnd_sig_list){
			n.show();
		}
	}

	void show_cluster_result(){
		fprintf(stderr, "========== BND signal clustering result ==========\n");
		fprintf(stderr, "Total clusters: %zu, total items: %zu\n",
		result.clusters.size(), bnd_sig_list.size());
		for (size_t c = 0; c < result.clusters.size(); ++c) {
			fprintf(stderr, "--- Cluster %zu (size: %zu) ---\n",
			c, result.clusters[c].size());
			for (size_t idx : result.clusters[c]) {
				bnd_sig_list[idx].show();
			}
		}
		fprintf(stderr, "==================================================\n");
	}
...
struct SV_CALLING_Handler{
	...
	struct LONG_range_CONTIG_ALN_HANDLER{

		bool successfully_modification;
		int match_base_left = 0;
		int match_base_right = 0;

		int mismatch_base_left = 0;
		int mismatch_base_right = 0;

		int INS_base_left = 0;
		int INS_base_right = 0;

		int INS_event_left = 0;
		int INS_event_right = 0;

		int DEL_base_left = 0;
		int DEL_base_right = 0;

		int DEL_event_left = 0;
		int DEL_event_right = 0;	

		void clear(){
			successfully_modification = false;
			match_base_left = 0;
			match_base_right = 0;

			mismatch_base_left = 0;
			mismatch_base_right = 0;
		...
		void add_cigar(int type, int cigar_len){
			if(0 == type || 7 == type){//match
				if(successfully_modification == false)
					match_base_left += cigar_len;
				else
					match_base_right += cigar_len;
			}
			else if(8 == type){//mismatch
				if(successfully_modification == false)
					mismatch_base_left += cigar_len;
				else
					mismatch_base_right += cigar_len;
			}
			else if(1 == type){//insertion
				if(successfully_modification == false){
					INS_base_left += cigar_len;
					INS_event_left ++;
				}
				else{
					INS_base_right += cigar_len;
					INS_event_right ++;
				}
			}
			else if(2 == type){//deletion
				if(successfully_modification == false){
					DEL_base_left += cigar_len;
					DEL_event_left ++;
				}
				else{
					DEL_base_right += cigar_len;
					DEL_event_right ++;
				}
			}
		}
		...
		void print(){
			std::cerr 
			<< "successfully_modification=" << successfully_modification
			<< "match_base_left=" << match_base_left 
			<< ";mismatch_base_left=" << mismatch_base_left 
			<< ";INS_base_left=" << INS_base_left 
			<< ";DEL_base_left=" << DEL_base_left 
			<< ";INS_event_left=" << INS_event_left 
			<< ";DEL_event_left=" << DEL_event_left
			<< ";match_base_right=" << match_base_right 
			<< ";mismatch_base_right=" << mismatch_base_right 
			<< ";INS_base_right=" << INS_base_right 
			<< ";DEL_base_right=" << DEL_base_right
			<< ";INS_event_right=" << INS_event_right 
			<< ";DEL_event_right=" << DEL_event_right 
			<< std::endl;
		}
	};
...
private:
	...
	LRS_Somatic_BND_Handler lrs_somatic_bnd_Handler;
...
private:
	...
	void SRS_GET_SIGNALS_FROM_READs(float & average_read_depth);
	...
	void LRS_assembly_load_read(bool print_log, RefRegion &main, Ass_Block & a_b, int min_LRS_read_length, BAM_handler *cur_LRS_read, bool loading_LRS_SUPP_READ);
	...
	bool assembly_load_read(bool print_log, RefRegion r1, RefRegion r2, RefRegion main,
			RefRegion supp, int minQUAL, uint MAX_load_reads, bool using_LRS, bool using_SRS, int min_LRS_read_length, BAM_handler *cur_LRS_read, bool loading_LRS_SUPP_READ);
	...
	/*************************************LRS FUNCTIONs**********************************************/
	bool LRS_assembly_variations(bool print_log, BAM_handler *cur_LRS_read, LRS_SIG &sig, RefRegion &loadRef,std::string &full_ref_str, std::vector<uint8_t> &bin_ref, int max_read_depth_assembly, bool loading_SUPP_READ);
	void LRS_SV_generating_Germline(bool print_log, LRS_SIG &sig, RefRegion &loadRef, std::string &full_ref_str, std::vector<uint8_t> &bin_ref,
			 std::vector<NOVA_SV_FINAL_RST_item> & SV_result_final, std::vector<TARGET_path_and_index> &target_path_idx_l, CONTIG_SELECTER &contig_selecter, Ass_Block &ass_block);
	void LRS_SV_generating_Somatic(bool print_log, LRS_SIG &sig,
			RefRegion &loadRef, std::string &full_ref_str, std::vector<uint8_t> &bin_ref, BAM_handler *LRS_Tumor_read,
			...
			std::vector<TARGET_path_and_index> &target_path_idx_l_Normal,CONTIG_SELECTER &contig_selecter_Normal, bam_hdr_t * lrs_tumer_header);
	void Somatic_generate_BND_SV(bool main_BP_is_left, bool same_direction, int main_BP_ref_chrID, int main_BP_ref_position,  std::vector<NOVA_SV_FINAL_RST_item> &SV_result_TMP_buff,
	...
	bool Somatic_local_full_cover_contig_UNIQ_check(bool print_log, CONTIG_SELECT &tc, std::string & full_contig_tc, 
	...
	void LRS_contig_candidate_generate(bool print_log, int minDepth_LRS, int KMER_len, int max_target_number);
	...
	void LRS_SIG_COLLECTION(bool print_log, BAM_handler &cur_read, LRS_SIG_Handler & sig_h,	float &average_read_depth, bool is_LRS_somatic);
	...
	void LRS_somatic_log_signal_header(const char *step_tag, const char *message, LRS_SIG &sig);
	void LRS_somatic_log_contig_collection(const char *step_tag, const char *message, LRS_SIG &sig,
	...
	void LRS_somatic_log_single_contig(const char *step_tag, const char *message, LRS_SIG &sig,
	...
	void LRS_somatic_log_realignment_result(const char *step_tag, const char *message, LRS_SIG &sig,
			uint haplotype_ID, CONTIG_SELECT &tc, const std::string &contig_seq,
...
public:
	...
	void init(RefHandler *ref_, KMER_ERROR_PROFILE_HANDLER * kmer_profile_handler, RefLocalRepeat *rlr_,
			...
			int preset_platform, int Contig_aln_preset, char * FC_BED_F, int OUT_MODE_int, std::string & command_with_time){
		this->random_phasing = random_phasing;
		this->output_small_var = output_small_var;
		this->preset_platform = preset_platform;
		this->Contig_aln_preset = Contig_aln_preset;
		...
		dbgHandler->init();
		ca.init(Contig_aln_preset);
		int somatic_same_ca_preset = PRESET_SOMATIC;
		somatic_same_ca.init(somatic_same_ca_preset);
		ga.init();
		...
		if(output_vcf_header && this->OUT_MODE_int != OUT_MODE_PURE_STR)
			NOVA_SV_FINAL_RST_item::write_to_vcf_header(vcf_w, cur_header, command_with_time);
		...
		//BND
		bnd_ass_block.init();
		ca_bnd.init(PRESET_M2);
		ca_bnd.setZdrop(1500, 1500);

		ca_re_locate.init(PRESET_M2);
    ...
    void SV_calling_in_region_main_pipeline(){
    	...
    	if(is_somatic_calling){
    		...
    		LRS_SIG_COLLECTION(print_log, LRS_Tumor_read, tumor_sig_h, average_read_depth_tumer, true);
    		...
    		LRS_SIG_COLLECTION(print_log, LRS_Normal_read, normal_sig_h, average_read_depth_normal, true);
    		...
    		int max_read_depth_assembly_tumer = MAX(average_read_depth_tumer*10, 100);
    		int max_read_depth_assembly_normal = MAX(average_read_depth_normal*8, 100);
    		...
    		for(int i = 0; i < signal_list_size; i++){
    			...
    			if(l_T[i].support_read_Num >= min_support_read_number_tumer){
    				...
    				//LRS_;
					//only set loading_SUPP_READ to true when calling Somatic SV
					fprintf(stderr, "### Assembly contig for tumors\n\n");
    				if(false == LRS_assembly_variations(print_log, &LRS_Tumor_read, l_T[i], loadRef, full_ref_str, bin_ref, max_read_depth_assembly_tumer, true)){
						LRS_somatic_log_signal_header("Step1", "tumor assembly finished: assembly failed, no tumor contig generated.", l_T[i]);
 						continue;
					}
    				...
    				if(LRS_Normal_BAM_F != NULL){
						fprintf(stderr, "### Assembly contig for normals\n\n");
						//only set loading_SUPP_READ to true when calling Somatic SV
						if(false == LRS_assembly_variations(print_log, &LRS_Normal_read, l_T[i], loadRef, full_ref_str, bin_ref, max_read_depth_assembly_normal, true)){
							if(false){
								LRS_somatic_log_signal_header("Step2",
									"tumor unique filtering skipped: normal assembly failed for current tumor signal.",
									l_T[i]);
					...
					bam_hdr_t * lrs_tumer_header = LRS_Tumor_read.file._hdr;
					
    				//SOMATIC SV generation:: todo::?????
    				LRS_SV_generating_Somatic(print_log, l_T[i], loadRef, full_ref_str, bin_ref, &LRS_Tumor_read,
    						result_LRS_final, tumor_sig_h.sa_handler,
							target_path_idx_l_Tumer,  contig_selecter_Tumer,
							target_path_idx_l_Normal, contig_selecter_Normal, lrs_tumer_header);
    			}
    		}
    	...
    	if(!is_somatic_calling && is_FC_REGION_LRS){
    		...
    		LRS_SIG_COLLECTION(print_log, LRS_read, lrs_sig_h, average_read_depth, false);
    	...
    	if(!is_somatic_calling && !is_FC_REGION_LRS){
            ...
            if(LRS_BAM_F != NULL){//handle ONT dataset
    			...
    			//remove duplication SVs in different reference regions
								print_log = true;
				if(print_log) showSVList(result_LRS_final, "\n\n before remove_LRS_duplications:\n");
    			LRS_remove_duplications(true, result_LRS_final);
    			if(print_log) showSVList(result_LRS_final, "\n\n After remove_LRS_duplications:\n");
    			//combine same SVs in different haplotype-s
    			//0/1 + 0/1 ---> 1/1
    			...
    			//re-genotyping LRS result using SRS reads when data available
    			if(SRS_BAM_F != NULL && !result_LRS_final.empty()){
    				fprintf(stderr, "genotypingTGS_Using_SRS begin:\n");
    				for(std::vector<NOVA_SV_FINAL_RST_item>::value_type &sv: result_LRS_final){
						//re-genotype SVs skip when with depth < 10 and ERR_PRONE platform
						//逻辑：当高错误率、低深度模式下：只有当分析区域不是VNTR时候，才调用re-GT
						bool sv_is_VNTR = false;
						rlr_result.clear();
						xassert(rlr->region_length != 0, "rlr->region_length == 0");
						rlr->search(sv.chr_ID, sv.st_pos, sv.st_pos + 10, rlr_result);
						if(!rlr_result.empty())
							sv_is_VNTR = true;

						if(average_read_depth < 10 && preset_platform == PRESET_ERR && sv_is_VNTR)
							continue;
						...
						if(sv.REF_perfect_support_by_SRS() == false && preset_platform == PRESET_ERR)
							continue;
            ...
            if(SRS_BAM_F != NULL){//debug code: loading SRS vcfs, mergin and output:// not used right now
                //handle SRS reads: clear result list:
            	...
            	{  //pure SRS mode: truly handle SRS reads
                    //get signals from SRS CRAM
					float average_read_depth = 0;
                    SRS_GET_SIGNALS_FROM_READs(average_read_depth);//S2:load reads and get signals
                    //not used right now
                    if(false)SRS_load_realignment_read_from_file();//S3:try to load realignment reads into UNMAPPED read list
					sig_para.reset_depth(average_read_depth);
                ...
                //remove SVs called by SRS data-set that is similar with LRS results
