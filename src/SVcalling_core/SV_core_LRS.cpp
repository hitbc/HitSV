...
void TARGET_path_and_index::get_the_contig_string(bool print_log, std::vector<UNITIG_INFO_item> &unitig_l,
	...
	for(int target_node_id = the_bg_node_id; target_node_id != -1; target_node_id = target_link_path[target_node_id].next_node_id){
		...
		if(print_log){
			fprintf(stderr, "uid %d, %f, %d\n", uid, unitig_l[uid].averageKmerDepth_read, unitig_l[uid].totalDepthRef);
		}
		...
		bool FULL_cover = (a.U_pos_bg == 0 && a.U_pos_ed == a.unitig_kmer_number);
		bool quality_is_high = true;
		if(FULL_cover){
			add_string += unitig_str.substr(kmer_size - 1, unitig_str.size() - kmer_size + 1);
			if(print_log) {
				for(int i = 0; i < add_string.size(); i++){
					if(add_string[i] != 'A' && add_string[i] != 'C' && add_string[i] != 'G' && add_string[i] != 'T')
						xassert(false, "");
				}
			}
			...
			//todo:: with ERROR, need to be fix: when old_read_id == -1, the first read is not the first read in the unitig.
		...
		if(with_outside_gap){
			...
			if(is_skip_depth_1_kmer == true)
				quality_is_high = false;
			if(print_log) fprintf(stderr, "3: %s\n", add_string.c_str());
		}

		if(is_skip_depth_1_kmer == true && unitig_l[uid].averageKmerDepth_read == 1)
			quality_is_high = false;
		...
		contig += add_string;
		if (quality_is_high) {
			contig_string_quality += std::string(add_string.size(), '1');
		} else {
			contig_string_quality += std::string(add_string.size(), '0');
		}
	...
	if(print_log){
		...
		fprintf(stderr, "final contig quality: %s\n",contig_string_quality.c_str());
	}
}

void TARGET_path_and_index::get_the_contig_string_somatic_mode(bool print_log, std::vector<UNITIG_INFO_item> &unitig_l,
		std::vector<std::string> & read_string, bool is_skip_depth_1_kmer){
	print_log = false;
	xassert(!unitig_l.empty(), "");
	int kmer_size = unitig_l[0].unitig.size()  - unitig_l[0].kmer_list.size() + 1;
	...
	for(int target_node_id = the_bg_node_id; target_node_id != -1; target_node_id = target_link_path[target_node_id].next_node_id){
		...
		if(FULL_cover){
			...
			int length_unitig = a.U_pos_ed - a.U_pos_bg;
			//todo:: with ERROR, need to be fix: when old_read_id == -1, the first read is not the first read in the unitig.
			bool read_switch = (a.ori_read_id != old_read_id && old_read_id != -1);
			if(read_switch){
				bg_pos_unitig = kmer_size - 1;
				length_unitig = a.U_pos_ed;
			}
...
void TARGET_path_and_index::show_read_list(const char * title_str, std::vector<ASS_reads_info> &ass_read_list, int haplotype_ID){
	...
	//show the read detail
	for(SUPPORT_READ_LIST_ITEM & r : support_read_l){
		ASS_reads_info &ai = ass_read_list[r.read_id];
		ai.show_LRS_info();
	}

	//primary read analysis
	int total_reads = 0;
	int primary_reads = 0;
	//MAPQ analysis
	std::map<int, int> mapq_count;
	double mapq_sum = 0.0;
	int mapq_mode = 0;
	int max_count = 0;

	// count the clip position
	int left_clip_count = 0;
	int right_clip_count = 0;
	std::map<int, int> left_pos_counts;
	std::map<int, int> right_pos_counts;
	int left_pos_sum = 0;
	int right_pos_sum = 0;

	int left_pos_min = INT_MAX;
	int left_pos_max = INT_MIN;

	int right_pos_min = INT_MAX;
	int right_pos_max = INT_MIN;
	...
	for(SUPPORT_READ_LIST_ITEM & r : support_read_l){
		...
		total_reads++;
		// PRIMARY alignment has flag bit 2 unset (not secondary, supplementary, or QC fail)
		if(!((ai.flag & 0x100) || (ai.flag & 0x800))){  // 0x100 = secondary alignment
			primary_reads++;
		}
		mapq_count[ai.MAPQ]++;
		mapq_sum += ai.MAPQ;

		bool is_clip_left = (ai.soft_left > MIN_CLIP);
		bool is_clip_right = (ai.soft_right > MIN_CLIP);	
		int left_position = ai.read_position;
		int right_position = ai.read_end;
		left_pos_min = std::min(left_pos_min, left_position);
		left_pos_max = std::max(left_pos_max, left_position);
		right_pos_min = std::min(right_pos_min, right_position);
		right_pos_max = std::max(right_pos_max, right_position);
		
		left_positions.push_back(left_position);
    	right_positions.push_back(right_position);

		// 统计左侧clip
		if(is_clip_left){
			left_clip_count++;
			int pos_bucket = left_position / 100;
			left_pos_counts[pos_bucket]++;
			left_pos_sum += pos_bucket;
		}
		
		// 统计右侧clip
		if(is_clip_right){
			right_clip_count++;
			int pos_bucket = right_position / 100;
			right_pos_counts[pos_bucket]++;
			right_pos_sum += pos_bucket;
		}
	}

	// 计算left_position的平均值和标准差
	double left_mean = 0.0, left_stddev = 0.0;
	if (!left_positions.empty()) {
		left_mean = std::accumulate(left_positions.begin(), left_positions.end(), 0.0) / left_positions.size();
		double left_variance = 0.0;
		for (int pos : left_positions) {
			left_variance += (pos - left_mean) * (pos - left_mean);
		}
		left_stddev = std::sqrt(left_variance / left_positions.size());
	}

	// 计算right_position的平均值和标准差
	double right_mean = 0.0, right_stddev = 0.0;
	if (!right_positions.empty()) {
		right_mean = std::accumulate(right_positions.begin(), right_positions.end(), 0.0) / right_positions.size();
		double right_variance = 0.0;
		for (int pos : right_positions) {
			right_variance += (pos - right_mean) * (pos - right_mean);
		}
		right_stddev = std::sqrt(right_variance / right_positions.size());
	}

	
	// count the average MAPQ value
	double mapq_avg = total_reads > 0 ? mapq_sum / total_reads : 0.0;
	
	// find the mode of MAPQ value
	for(const auto& pair : mapq_count) {
		if(pair.second > max_count) {
			max_count = pair.second;
			mapq_mode = pair.first;
		}
	}
	...
	for(auto const & [pos, count] : left_pos_counts){
		if(count > left_max_count){
			left_max_count = count;
			left_mode = pos;
		}
	}
	double left_avg = left_clip_count > 0 ? (double)left_pos_sum / left_clip_count : 0.0;

	// 计算右侧clip的众数和平均值
	int right_mode = 0;
	int right_max_count = 0;
	for(auto const & [pos, count] : right_pos_counts){
		if(count > right_max_count){
			right_max_count = count;
			right_mode = pos;
		}
	}
	double right_avg = right_clip_count > 0 ? (double)right_pos_sum / right_clip_count : 0.0;

	std::cerr << title_str
		<< "Left_2_right=" << left_pos_min << "-" << right_pos_max << ";"
		<< "Contig_range=" << (right_pos_max - left_pos_min) << ";"
		<< "L_R_clip_count=" << left_clip_count << "-" << right_clip_count << ";"
		<< "L_R_clip_ratio=" << (static_cast<double>(left_clip_count) / (total_reads+0.001)) * 100 << "%-" << (static_cast<double>(right_clip_count) / (total_reads+0.001)) * 100 << "%;"
		<< "L_R_clip_mode_(bucket)=" << (left_mode * 100) << "-" << (right_mode * 100) << ";"
		<< "L_R_clip_average_(bucket)=" << static_cast<int>(left_avg * 100) << "-" << static_cast<int>(right_avg * 100) << ";";

	std::cerr << std::fixed << std::setprecision(2);
	std::cerr << "Left_position_Mean=" << left_mean << ";"
		<< "Left_position_StdDev=" << left_stddev << ";"
		<< "Right_position_Mean=" << right_mean << ";"
		<< "Right_position_StdDev=" << right_stddev << ";"
		<< "Left_position_range=" << left_pos_min << "-" << left_pos_max << ";"
		<< "Right_position_range=" << right_pos_min << "-" << right_pos_max << ";";
		  
	// Print the primary read statistics
	// show the MAPQ statistics
	std::cerr << "Total_reads=" << total_reads << ";"
			<< "Primary_reads=" << primary_reads << ";"
			<< "Primary_ratio=" << std::fixed << std::setprecision(2) 
			<< (static_cast<double>(primary_reads) / total_reads * 100) << "%;"
			<< "MAPQ_Average=" << std::fixed << std::setprecision(2) << mapq_avg << ";"
			<< "MAPQ_Mode=" << mapq_mode << ";"
			<< "appeared_times=" << max_count << ";";
...
void TARGET_path_and_index::build(std::vector<READ_PATH_COMBINE_ITEM> & target_seq, int read_id, std::string &read_name, int read_mapping_pos,
	...
	support_read_l.emplace_back(read_id, read_name, read_mapping_pos);
...
void CONTIG_SELECTER::contig_basic_count(bool print_log, std::vector<TARGET_path_and_index> &target_path_idx_l, std::vector<READ_MATCH_NUM> & rml, std::vector<ASS_reads_info> &ass_read_list){
	...
	for(uint cur_target_ID = 0; cur_target_ID < target_path_idx_l.size(); cur_target_ID++){
		...
		for(SUPPORT_READ_LIST_ITEM & s : target_path_idx_l[cur_target_ID].support_read_l){
			if(s.is_best_target == true){
				...
				ASS_reads_info &ai = ass_read_list[s.read_id];
				total_mapq += ai.MAPQ;
				total_mapq_read_n ++;
		...
		float avg_mapq = (total_mapq_read_n == 0)?0:(float)total_mapq / (float)total_mapq_read_n;
		...
		select_list.back().set(
				...
				target_path_idx_l[cur_target_ID].contig.size(), avg_mapq);
	}
}
...
bool CONTIG_SELECTER::select_contig(bool print_log, int preset_platform){
	...
	//remove low ave reads mapq path:
	for(uint i = 0; i < select_list.size(); i++){
		CONTIG_SELECT *ch = &(select_list[i]);
		if(ch->avg_mapq < 6){
			ch->support_read_number = 0;//MAGIC number
		}
		if(ch->full_cover_read_n < 1){
			ch->support_read_number = 0;//MAGIC number
		}
	}
	...
	//fprintf(stderr, "Contig select: S1\n\n");
	...
	//fprintf(stderr, "Contig select: S2\n\n");
	//for(auto & s :select_list)	s.show(true);
	...
	//上面的对：
	if(print_log){
		fprintf(stderr, "Contig select: S3 \n\n");
		for(auto & s :select_list)	s.show(true);
	}
	...
	//fprintf(stderr, "Contig select: S4\n\n");
	//for(auto & s :select_list)	s.show(true);
...
void SV_CALLING_Handler::LRS_SIG_COLLECTION(bool print_log, BAM_handler &cur_read, LRS_SIG_Handler & sig_h,
		float &average_read_depth, bool is_LRS_somatic) {
	...
	while (bam_next(c_b)) {
		...
		if(false){
			std::string qname = bam_get_qname(br);
			if(qname == "d5bee653-778b-44ff-86ae-08d98b1df11e"){
				fprintf(stderr, "Read Begin NAME %s \n", bam_get_qname(br));;
			}
		}
		...
		cur_read.store_LRS_ReadCore(br->core.l_qseq, cur_read.storeReadBuff,
				bam_get_qual(br), br->core.n_cigar, bam_get_cigar(br), bam_get_qname(br), primary_chrID, primary_pos, br->core.flag,
				 br->core.pos, bam_is_fwd_strand(br), br->core.qual);		//add the supplementary data:
		...
		for (int i = cigar_idx; i < cigar_end; i++) {
			...
			{
				if (i == cigar_idx) { soft_left = p->length; } else { soft_right = p->length; }
				//at lease 1000 bp
				if (p->length > 1000) {					
					with_long_CLIP = true;
					if (print_log)
				...
				total_read_len += p->length;
				//only store the hard clip in somatic mode:
				if(using_clip_signal && is_LRS_somatic){
					CLIP_sig.emplace_back();
					CLIP_sig.back().store(LRS_SIG_CLIP_RIGHT,
							br->core.tid, br->core.pos + ref_i,
							br->core.pos + ref_i);				//END==POS
		...
		cur_read.read_list.back().soft_left = soft_left;
		cur_read.read_list.back().soft_right = soft_right;
		...
		if(using_SA_signal && with_long_CLIP){
			...
			if(SA_tag_char != NULL){
				...
				bool bam_is_primary = ((false == bam_is_supplementary(br)) && (false == bam_is_second(br)));
				bool mapping_direction = (br->core.flag & BAM_STRAND) == 0;
				...
				sig_h.sa_handler.store(
						...
						SA_tag_char, total_read_len, ref_handler->getIdx(), mapping_direction == 1);
			}
		}
	}
...
void SV_CALLING_Handler::LRS_SV_CALLING_germline(bool print_log,
	...
	int max_read_depth_assembly = MAX(average_read_depth * 5, 100);
	max_read_depth_assembly = MIN(max_read_depth_assembly, 1000);
	...
	for(int i = 0; i < signal_list_size; i++){
		...
		if(LRS_assembly_variations(print_log, &LRS_read, l[i], loadRef, full_ref_str, bin_ref, max_read_depth_assembly, false) == false){
			if(output_pure_contig){
		...
		if(output_pure_contig){
			...
			fprintf(stderr, "SV contig_selecter size: %ld\n", contig_selecter.select_list.size());
			for(uint haplotype_ID = 0; haplotype_ID < contig_selecter.select_list.size(); haplotype_ID++){
				if(0 == contig_selecter.select_list[haplotype_ID].support_read_number)		{
					fprintf(stderr, "SKIP: support read number is 0");
					continue;
				}
				if(false == contig_selecter.select_list[haplotype_ID].is_final_path)		{
					fprintf(stderr, "SKIP: not final path");
					continue;
				}
				...
				{
					fprintf(stderr, "SKIP: contig length is over long");
					continue;
				}
				fprintf(stderr, "PASS: contig %d\n", haplotype_ID);
				true_out_num ++;
			}
			if(0 == true_out_num){
				fprintf(vcf_w, "REF_REGION=%s:%d-%d\t",	faidx_iseq(c_ref_idx, loadRef.chr_ID), (int)l[i].POS,(int)l[i].END);
			...
			LRS_SV_generating_Germline(print_log, l[i], loadRef, full_ref_str, bin_ref, result_LRS_final, 
				target_path_idx_l, contig_selecter, ass_block);
	}
}
	...
	for(uint i = 0; i < query.size();i++){
		...
		for(uint j = 0; j < target.size();j++){
			...
			if(SRS_r->chr_ID != LRS_r->chr_ID || ABS_POS > 300)
				continue;
			if(print_log){
				fprintf(stderr, "SRS results is removed:\n");
...
void SV_CALLING_Handler::Hybrid_regenotype_NGS_USING_TGS_data(bool print_log){
	...
	//merging and remove duplication SRS results that similar with LRS results
	Hybrid_remove_similar_SVs_in_Target(print_log, result_SRS_INDEL, result_LRS_final);
	//
	//genotyping the SRS result using LRS data set
	for(std::vector<NOVA_SV_FINAL_RST_item>::value_type &sv: result_SRS_INDEL ){
		if(print_log) showSV(sv, "GT SRS SVs using LRS reads[Before]:\n"); //show all the SVs
		...
		for(int mode = 0; mode < 2; mode ++){
			...
			int check_edge = 500;
			int check_check_position_st = check_position - check_edge;
			if(check_check_position_st < 0)	check_check_position_st = 0;
			RefRegion r(sv.chr_ID, check_check_position_st, check_position + check_edge);
			if(print_log) r.show();
		...
		//for VNTR-SRS-data:
		if(sv.isVNTRcallerRst && sv.LRS_INFO.not_supp_read_n > 0 && sv.LRS_INFO.supp_read_n == 0){
			sv.setGenotype_directly(0);
		}

		if(print_log) showSV(sv,  "GT SRS SVs using LRS reads[After]:\n");
	}
	if(print_log) fprintf(stderr, "Show SRS result after merging\n\n");
}
