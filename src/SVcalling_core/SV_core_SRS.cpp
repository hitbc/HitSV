...
void SV_CALLING_Handler::SRS_GENOTYPING_INS_DEL(){
    ...
    for(std::vector<NOVA_SV_FINAL_RST_item>::value_type &sv: result_SRS_INDEL ){
        if(ABS(sv.SV_length) < this->MIN_sv_len) continue;
		...
		if(sv.is_duplication_sv) continue;
...
bool SV_CALLING_Handler::SRS_INV_ASSEMBLY(SVE &c_sve, int &ASS_SIGNAL_NUM){
    ...
    RefRegion main_region(c_sve.r1.chr_ID, main_st_pos,main_pos + edge_len);
    RefRegion supp_region(c_sve.r2.chr_ID, supp_st_pos,supp_pos + edge_len);
