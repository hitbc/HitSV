...
struct FA_STATUS{
	...
	int run_ana_core(char * fa_fn_in, bool output_bed_format){
			...
			for(;beginPos < chrLength; beginPos += REGION_STEP_LEN){
				...
				if(withloop){
					if(output_bed_format == false){
						...
						fprintf(stdout, "%s\t%d\t%d\t%d\n", chrName, beginPos, endPos, kmerSizeF);
					}
				}
			}
			free(c_reference);
	...
	int run_ana(int argc, char *argv[]){
		...
		return run_ana_core(fa_fn_in, false);
	}

	int run_ana_bed(int argc, char *argv[]){
		//bs->load(nova_para->statsFileName);
		//	//PART5: for each choromosome
		if(argc != 1){
			fprintf(stderr, "WARNING: the reference file is needed\n\n");
		...
		char * fa_fn_in = argv[0];//separate by ','
		return run_ana_core(fa_fn_in, true);
	}
