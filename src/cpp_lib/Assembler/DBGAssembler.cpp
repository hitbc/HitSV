...
void UNITIG_INFO_item::setString(std::vector<DBG_INFO_item *> &id2info)
{
	...
	if(false){
		for(int i = 0; i < unitig.size(); i++){
			if(unitig[i] != 'A' && unitig[i] != 'C' && unitig[i] != 'G' && unitig[i] != 'T')
				xassert(false, "");
		}
	}
...
bool MainDBGHandler1::buildDBG(int minDepthNGS, int minDepthTGS)
{
	...
	// set begin and end
	std::vector<int> headKmers;
	std::vector<int> tailKmers;
	for (DBG_INFO_item *kmerIdx_i : id2info)
	{
		...
		if (kmerIdx_i->is_UNITIG_begin)
			headKmers.emplace_back(kmerIdx_i->kmerId);
		if (kmerIdx_i->is_UNITIG_end)
			tailKmers.emplace_back(kmerIdx_i->kmerId);
	}
