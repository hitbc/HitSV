...
struct KmerCounter
{
	...
	void show(std::string &kmerStr) { fprintf(stderr, "%s: [ %d | %d ]\n", kmerStr.c_str(), read_Srs_n, refN); }
	bool is_low_depth(int minDepthNGS, int minDepth_LRS) {
		 return (read_Srs_n < minDepthNGS && refN == 0 && read_Lrs_n < minDepth_LRS);
		 }
	int getReadDepth(int LOW_Depth, int minDepth_LRS)
	{
		int read_count = 0;
		if (read_Srs_n >= LOW_Depth)
			read_count = read_Srs_n;
		if (read_Lrs_n >= minDepth_LRS)
			read_count += MAX(LOW_Depth, read_Lrs_n);
		return read_count;
	}
};
...
struct UNITIG_INFO_item
{
	...
	void show_link(std::vector<DBG_INFO_item *> &id2info)
	{
		// fprintf(stderr, "\n");
		...
		//	return;
		//}
		fprintf(stderr, "[BASE PART][ID %3d total %5ld, AVE_DP: %5.2f\t",
				id, kmer_list.size(), (float)totalDepthRead / kmer_list.size());
		...
		fprintf(stderr, "[String part]");
		fprintf(stderr, "\t%s\t:", unitig.c_str());
		fprintf(stderr, "\n");
	}

	std::vector<ReadPos> &getAllReadTGSHead(std::vector<DBG_INFO_item *> &id2info)
	{
		return id2info[kmer_list[0]]->kcp->LRS_rp;
	}
	std::vector<ReadPos> &getAllReadTGSTail(std::vector<DBG_INFO_item *> &id2info)
	{
		return id2info[kmer_list.back()]->kcp->LRS_rp;
	}

	// only add ReadPos the appeared in head but not tail
	void getAllReadTGS_head_only(std::vector<DBG_INFO_item *> &id2info, std::vector<ReadPos> &result)
	{
		std::set<int> combine;
		result.clear();
		for (ReadPos &rp : id2info[kmer_list.back()]->kcp->LRS_rp)
		{
			combine.insert(rp.readID);
		}

		int head_end_pos_offset = kmer_list.size() - 1;
		for (ReadPos &rp : id2info[kmer_list[0]]->kcp->LRS_rp)
		{
			std::pair<std::set<int>::iterator, bool> r = combine.insert(rp.readID);
			if (true == r.second)
			{
				result.emplace_back(rp);
				result.back().readPos += head_end_pos_offset;
			}
		}
	}

	void getAllReadTGSAll(std::vector<DBG_INFO_item *> &id2info, std::vector<ReadPos> &result)
	{
		std::set<int> combine;
		result.clear();
		int head_end_pos_offset = kmer_list.size() - 1;
...
struct Path
{
	...
	std::vector<ReadPos> removed_read_l;
	...
	{
		if (a.bgnUID != b.bgnUID)
			return a.bgnUID < b.bgnUID;
		if (a.endUID != b.endUID)
			return a.endUID < b.endUID;
		if (a.unitigPL.size() != b.unitigPL.size())
			return a.unitigPL.size() < b.unitigPL.size();
		if (a.SV_len != b.SV_len)
			return a.SV_len < b.SV_len;
		if (a.ID_SUM != b.ID_SUM)
			return a.ID_SUM < b.ID_SUM;
	...
	static inline int cmp_by_idSUM(const Path &a, const Path &b)
	{
		...
		if (a.endUID != b.endUID)
			return a.endUID < b.endUID;
		if (a.UNIQ_ID_product != b.UNIQ_ID_product)
			return a.UNIQ_ID_product < b.UNIQ_ID_product;
		if (a.UNIQ_ID_SUM != b.UNIQ_ID_SUM)
			return a.UNIQ_ID_SUM < b.UNIQ_ID_SUM;
		if (a.SV_len != b.SV_len)
			return a.SV_len < b.SV_len;
		return a.type < b.type;
	}
...
struct AB_SuggsetHandler
{
									...
									int bgnUID, int endUID, int adjustRate, float maxError)
	{
		...
		for (A_B_para_suggest &abs : *suggsetMode)
		{
			...
			for (NodeAnalysisINFO &na : nfl)
			{
				if (na.step_Count == 0)
					continue;
				if (!usingHeadNode && (na.up->id == bgnUID || na.up->id == endUID))
					continue;
	...
	bool getSVLen(AB_SuggsetItem *bsetS, int basicSVLength, int stepSVLength, int &suggestFinalSVLength, bool &isHOM, bool using_Linear)
	{
		...
		else if (bsetS->A == 1 && bsetS->B == 1)
		{ // 1:1
			suggestFinalSVLength = basicSVLength + stepSVLength;
		}
		else if (bsetS->A == 1 && bsetS->B <= 4)
		{ // 1:?(2,3,4)
...
struct PathFilter
{
	...
	{
		for (int i = 0; i < cro.readOffset; i++)
			fprintf(stderr, " ");
		...
		bool showRst, bool isDEL, int SV_contig_bg, int SV_contig_ed)
	{
		...
		for (int curUniIdx = 0; curUniIdx < usize; curUniIdx++)
		{
			UNITIG_INFO_item *up = unitigPL[curUniIdx];
			int search_begin = 0;
			int search_end = up->kmer_list.size();
			if (curUniIdx == 0)
			{ // the begin
		...
		for (ReadOffset &cro : ro)
		{
			...
			if (readIsUsed)
			{
				...
				if (showRst)
					showRead(contig_str, cro, rstr, rlen, contigSize);
			}
		}
				   ...
				   float linearMIN_AVE_KMER_DEPTH, bool showRst, bool using_Linear)
	{
		...
		{ // path is linear path
			uniqUntigSet.clear();
			int bgn_check = 1;
			int end_check = aveKmerNumL.size() - 1;
			if (aveKmerNumL.size() <= 3)
			{
				bgn_check = 0;
				end_check = aveKmerNumL.size();
			}
...
struct KeyNodePath
{
					   ...
					   std::vector<DBG_INFO_item *> &id2info, std::vector<UNITIG_INFO_item> &unitig_l, int &unusedPathN, bool showLog)
	{
		...
		for (int mode = 0; mode < 2; mode++)
		{
			...
			for (uint i = 1; i < ch->unitigPL.size(); i++)
			{
				...
				if (usedSignal.find(curID) == usedSignal.end())
				{
					...
					if (showLog)
						fprintf(stderr, "[%d(%f: %.4f %ld)--%d(%f: %.4f %ld): %05f]",
		...
		for (int mode = 0; mode < 2; mode++)
		{
			...
			for (uint i = 1; i < ch->unitigPL.size(); i++)
			{
				if (i >= 1)
				{
					uint64_t link2 = ch->unitigPL[i - 1]->id;
					link2 <<= 10;
		...
		if (ABS_U(h1->SV_len, h2->SV_len) < 5)
		{ // same length
			score = score * 0.7;
		}
		else if (ABS(h1->SV_len) < 5 && ABS(h2->SV_len) < 5)
		{ // 0/0
			score = score * 0.6;
		}
		else if (ABS(h1->SV_len) < 5 || ABS(h2->SV_len) < 5)
								   ...
								   std::string &alt_str)
	{
		...
		for (int i = contig_begin; i < contig_end; i++)
		{
			...
			for (ReadPos &r : support_read_l)
			{
				...
				if (false)
					fprintf(stderr, "[%d:%d:%c:%d] ", r.readID, r.readPos + i, reads[r.readID][r.readPos + i], RunLength_count_read[r.readID][r.readPos + i]);
			}
			if (read_s == 0)
				break;
	...
	int main_path_selection(bool print_log, std::vector<DBG_INFO_item *> &id2info, int &total_read_n, int &unknown_read_n)
	{
		...
		if (p_l.size() > 0)
		{
			...
			{
				Path *ch = &(p_l[i]);
				if (ABS(cur_SV_len - ch->SV_len) < 10)
				{ // similar length
			...
			if (final_path_number > 2)
			{ // re-select when with too
		...
		std::vector<UNITIG_INFO_item> &unitig_l, Genotyping_read_aligner *gra, Bam_file *bam_f)
	{
		...
		else
		{
			if (pathD_l[0].depthDiffScore > 0.3)
				resultIsPassFilter = false;
			float S1 = pathD_l[0].p1->SV_len;
			S1 = ABS(S1);
			float S3 = pathD_l[0].p2->SV_len;
			S3 = ABS(S3);
			if (S1 > S3)
				std::swap(S1, S3);
			...
			{
				float S2 = pathD_l[i].p1->SV_len;
				S2 = ABS(S2);
				float S4 = pathD_l[i].p2->SV_len;
				S4 = ABS(S4);
				if (S2 > S4)
					std::swap(S2, S4);
		...
		for (int mode = 0; mode < 2; mode++)
		{
			...
			if (showLogs)
				fprintf(stderr, "Alignment Begin\n");
			std::vector<uint8_t> bin_contig;
			store_bin_contig(contig, bin_contig);
			ca->align_non_splice_default_ref(&(bin_contig[0]), bin_contig.size(), refPosBgn, refPosEnd, 0);
			refPosBgn += ca->adjustCIGAR();
			ca->printCIGAR(stderr);
			if (showLogs)
				ca->printf_alignment_detail(stderr, refPosBgn, NULL, 0);
			int match_size;
			...
			while (SV_final_size < SVE_SVs.size())
			{
				...
				if (false)
				{
					...
					for (Path *zeroP : zeroPath_l)
					{
						...
						if (main.region_overlap(supp))
						{
							main.Combine(supp, true);
							region_is_overlap = true;
						}
						std::cerr << "Region 1: " << main;
						if (!region_is_overlap)
							std::cerr << "Region 2: " << supp;
						std::cerr << std::endl;
						...
						for (int region_ID = 0; region_ID < 2; region_ID++)
						{ // mode == 0 for region1, mode == 1 for region2
							fprintf(stderr, "region_ID %d\n", region_ID);
							if (region_is_overlap == true && region_ID == 1)
								continue;
							R_region region;
							...
							while (bam_next(bam_f))
							{
								...
								{
									if (false)
										fprintf(stderr, "@Read name: %s Overlap_mode %d; SKIP\n", bam_qname(br), overlap_mode);
									continue;
								}
								...
								int true_read_pos_bg;
								int true_read_pos_ed;
								...
								if (showLogs)
									fprintf(stderr, "\ttrue_read_pos_bg %5d, contig_pos_bp1 %5d, read_in_contig_st_pos_bp1_rbg %5d\t", true_read_pos_bg, contig_pos_bp1, read_in_contig_st_pos_bp1_rbg);
								if (showLogs)
									fprintf(stderr, "\ttrue_read_pos_bg %5d, contig_pos_bp1 %5d, read_in_contig_st_pos_bp1_rbg %5d\t", true_read_pos_bg, contig_pos_bp2, read_in_contig_st_pos_bp2_rbg);
								...
								else if (overlap_mode == 3)
								{
									...
									{
										read_skip_left = read_skip_left_bp2;
										read_skip_right = read_skip_right_bp2;
								...
								if (read_contig_score > read_ori_score + 4 && read_contig_score > min_score)
									region_support_number[region_ID][0]++;
								else if (read_contig_score + 4 < read_ori_score && read_ori_score > min_score)
									region_support_number[region_ID][1]++;
								else
									region_support_number[region_ID][2]++;
							}
						}
...
struct READ_PATH_ITEM
{
	...
	int read_pos;
	int kmer_number;
	int unitig_kmer_number;

	void set(int UID, int U_pos, int read_pos, int kmer_number, int unitig_kmer_number)
	{
		this->UID = UID;
		this->U_pos = U_pos;
		this->read_pos = read_pos;
		this->kmer_number = kmer_number;
		this->unitig_kmer_number = unitig_kmer_number;
		if (UID == -1)
			this->unitig_kmer_number = -1;
		is_combined = false;
		xassert(unitig_kmer_number == -1 || U_pos + kmer_number <= unitig_kmer_number, "");
		//fprintf(stderr, "RP:set: ");
		//this->show();
	}

	void show()
	{
		fprintf(stderr, "|%d(U_pos:%d:R_pos:%d)%d:%d--\t", UID, U_pos, read_pos, kmer_number, unitig_kmer_number);
	}

	void show_simple(int &old_uid)
	{
		fprintf(stderr, "#%d\t", UID);
	}
};

struct READ_PATH_COMBINE_ITEM
{
	int UID;
	int U_pos_bg;
	int U_pos_ed;
	...
	int gap_kmer_num_outside; // gap out of UNITIG
	int gap_kmer_num_inside;  // gap within the UNITIG

	int read_id;

	bool query_node_with_gap()
	{
		if (gap_kmer_num_outside != 0 || U_pos_bg != 0 || U_pos_ed != unitig_kmer_number)
			return true;
		return false;
	}
	void init(READ_PATH_ITEM &com, int read_id)
	{
		this->read_id = read_id;
		this->UID = com.UID;
		this->U_pos_bg = com.U_pos;
		this->read_pos_bg = com.read_pos;

		this->U_pos_ed = com.U_pos + com.kmer_number;
		this->read_pos_ed = com.read_pos + com.kmer_number;

		if (UID == -1)
		{
			unitig_kmer_number = -1;

			gap_kmer_num_outside = com.kmer_number;
			gap_kmer_num_inside = 0;

			match_kmer_num = 0;
		}
		else
		{
			gap_kmer_num_outside = 0;
			gap_kmer_num_inside = 0;
			unitig_kmer_number = com.unitig_kmer_number;
			match_kmer_num = com.kmer_number;
		}
		com.is_combined = true;

		//fprintf(stderr, "INIT:read_id: %d\n", read_id);
		//com.show();
		//this->show();
		//fprintf(stderr, "INIT:END:\n");
		xassert(unitig_kmer_number == -1 || this->U_pos_ed <= unitig_kmer_number, "");
	}

	bool combine(READ_PATH_ITEM &com)
	{
		if (com.is_combined)
			return false;
		if (com.UID == -1)
		...
		{
			int gap_num = (com.U_pos - this->U_pos_ed) - (com.read_pos - this->read_pos_ed);
			if(ABS(gap_num) > 50)
				return false;

			if (this->U_pos_ed > com.U_pos)
				return false;
			else if (unitig_kmer_number * 2 < gap_kmer_num_inside + com.kmer_number + gap_kmer_num_outside)
			{
				return false;
			}
			else
			{
				gap_kmer_num_inside += gap_kmer_num_outside;
				gap_kmer_num_outside = 0;
				this->U_pos_ed = com.U_pos + com.kmer_number;
				this->read_pos_ed = com.read_pos + com.kmer_number;
				match_kmer_num += com.kmer_number;
				this->unitig_kmer_number = unitig_kmer_number;
				com.is_combined = true;
				xassert(this->U_pos_ed <= unitig_kmer_number, "");
				return true;
			}
		}
		//		else if(this->U_pos_ed + com.kmer_number + 10 < unitig_kmer_number){
		//			gap_kmer_num_outside += com.kmer_number;
		//			com.is_combined = true;
		//			return true;
		//		}
	...
	void show()
	{
		fprintf(stderr, "|%d(%d-%d);(%d-%d);%d:%d:%d:%d--\t", UID, U_pos_bg, U_pos_ed, read_pos_bg, read_pos_ed, match_kmer_num, unitig_kmer_number, gap_kmer_num_inside, gap_kmer_num_outside);
	}
	void show_simple()
	{
		fprintf(stderr, "%d--", UID);
	...
	void show1()
	{
		fprintf(stderr, "%d(%d:%d:%d)-\t", UID, match_kmer_num, gap_kmer_num_inside, gap_kmer_num_outside);
	}
};

struct READ_PATH
{
	std::vector<READ_PATH_ITEM> path;
	std::vector<READ_PATH_COMBINE_ITEM> combine_path;
	int total_match_number;
	void show_read_path(int read_id, int read_len)
	{
		fprintf(stderr, "SIMPLE\t%d\t(%d)\t", read_id, read_len);
		for (READ_PATH_ITEM &rp : path)
			rp.show();
		fprintf(stderr, "\n");
	}

	void show_combine_read_path(int read_id, int read_len)
	{
		fprintf(stderr, "Read\t%d\t(len:%d,match:%d)\n", read_id, read_len, total_match_number);
		for (uint query_node_id = 0; query_node_id < combine_path.size(); query_node_id++)
		{
			fprintf(stderr, "ID:%d R_POS:%d-%d UPOS:%d-%d UID:%d U_LEN:%d DIFF:%d (%d:%d:%d)-->\n",
					query_node_id,
					combine_path[query_node_id].read_pos_bg,
					combine_path[query_node_id].read_pos_ed,
					combine_path[query_node_id].U_pos_bg,
					combine_path[query_node_id].U_pos_ed,
					combine_path[query_node_id].UID,
					combine_path[query_node_id].unitig_kmer_number,
					combine_path[query_node_id].unitig_kmer_number - combine_path[query_node_id].match_kmer_num - combine_path[query_node_id].gap_kmer_num_inside,
					combine_path[query_node_id].match_kmer_num,
					combine_path[query_node_id].gap_kmer_num_inside,
					combine_path[query_node_id].gap_kmer_num_outside);
		}
		fprintf(stderr, "\n");
	}
};

struct ASS_TARGET_ITEM
{
	int UID;
	int U_pos_bg;
	int U_pos_ed;
	...
	int node_idx_position_in_path;
	int node_base_position_in_path;

	std::vector<READ_PATH_COMBINE_ITEM *> query_node_info;

	bool target_node_with_gap()
	{
		if (gap_kmer_num_outside != 0 || U_pos_bg != 0 || U_pos_ed != unitig_kmer_number)
			return true;
		return false;
	...
	void set(int read_id, READ_PATH_COMBINE_ITEM *ori, int node_position_in_path, int node_base_position_in_path)
	{
		query_node_info.clear();
		query_node_info.emplace_back(ori);

		UID = ori->UID;
		U_pos_bg = ori->U_pos_bg;
		U_pos_ed = ori->U_pos_ed;

		ori_read_id = read_id;
		read_pos_bg = ori->read_pos_bg;
		read_pos_ed = ori->read_pos_ed;

		kmer_number = ori->match_kmer_num;
		unitig_kmer_number = ori->unitig_kmer_number;
		xassert(unitig_kmer_number == -1 || U_pos_ed <= unitig_kmer_number, "");
		gap_kmer_num_outside = ori->gap_kmer_num_outside;
		gap_kmer_num_inside = ori->gap_kmer_num_inside;

		next_node_id = -1;
		prev_node_id = -1;
		this->node_idx_position_in_path = node_position_in_path;
		this->node_base_position_in_path = node_base_position_in_path;
	}
};

struct SDP_MATCH_ITEM
{
	int target_node_id;
	READ_PATH_COMBINE_ITEM *read_node;
	int diff; // = (t1-t2)-(q1-q2)
	int max_score;
	int max_previous_node; // the suffix_node of SDP result when get the max score;
	bool target_node_gap;
	bool query_node_gap;

	int target_len_total;
	int query_len_total;

	bool node_node_update()
	{
		return (target_node_gap == true && query_node_gap == false);
	}
	void set(int target_node_id, READ_PATH_COMBINE_ITEM *read_node)
	{
		this->target_node_id = target_node_id;
		this->read_node = read_node;
		max_score = -1;
		max_previous_node = -1;
		diff = MAX_int32t;
	}

	void show(std::vector<ASS_TARGET_ITEM> &target_link_path)
	{
		ASS_TARGET_ITEM &t = target_link_path[target_node_id];
		fprintf(stderr,
				"q_UID%d, t_UID%d, "
				"r_pos %d, t_pos: %d"
				"(%d:%d) %d\t",
				read_node->UID, t.UID,
				read_node->read_pos_bg, t.node_base_position_in_path,
				max_score, max_previous_node, diff);
		fprintf(stderr, "(%d:%d:%d:%d)\n", t.kmer_number, t.gap_kmer_num_inside, t.gap_kmer_num_outside, t.node_idx_position_in_path);
	}
};
...
struct PathReadFilter
{
	...
	bool isPathFilter(Path &p, std::map<uint64_t, int> &usedReadPath, bool print_log)
	{
		...
		for (uint i = 0; i < p.unitigPL.size(); i++)
		{
			...
			if (i >= 1)
			{
				uint64_t link2 = p.unitigPL[i - 1]->id;
				link2 <<= 10;
	...
	int get_read_kmer_UNITIG_ID(int read_pos, const std::string &reads, int &unitigPos,
								std::unordered_map<std::string, DBG_INFO_item> &kmerIdx)
	{
		unitigPos = -1;
		if (reads.size() < read_pos + KMER_len)
			return -1;
		const std::string word(reads.substr(read_pos, KMER_len));
		std::unordered_map<std::string, DBG_INFO_item>::iterator it = kmerIdx.find(word);

		if (it != kmerIdx.end())
		{
			unitigPos = it->second.unitigPos;
			return it->second.unitigId;
		}
		else
			return -1;
	}

	void get_string_path(bool print_log, std::unordered_map<std::string, DBG_INFO_item> &kmerIdx,
						 std::vector<UNITIG_INFO_item> &unitig_l, int KMER_len, std::string &reads, READ_PATH &read_path, int read_id)
	{
		std::vector<READ_PATH_ITEM> &rp = read_path.path;
		std::vector<READ_PATH_COMBINE_ITEM> &rp_com = read_path.combine_path;
		rp.clear();
		rp_com.clear();
		...
		int mapped_kmer_number = 0;
		int mapped_uid = -1;
		int mapped_unitigPos = -1;
		int uintig_kmer_len = -1;
		for (unsigned j = 0; j < kmer_number; ++j)
		{
			int unitigPos;
			int unitigId = get_read_kmer_UNITIG_ID(j, seq, unitigPos, kmerIdx);
			if (mapped_uid != unitigId || unitigPos != mapped_kmer_number + mapped_unitigPos)
			{
				rp.emplace_back();
				rp.back().set(mapped_uid, mapped_unitigPos, j - mapped_kmer_number, mapped_kmer_number, uintig_kmer_len);
				mapped_uid = unitigId;
				mapped_kmer_number = 1;
				mapped_unitigPos = unitigPos;
				uintig_kmer_len = (mapped_uid >= 0) ? unitig_l[mapped_uid].kmer_list.size() : (-1);
			}
			else
			{
				mapped_kmer_number++;
		...
		if (mapped_kmer_number != 0)
		{
			rp.emplace_back();
			rp.back().set(mapped_uid, mapped_unitigPos, kmer_number - mapped_kmer_number, mapped_kmer_number, (mapped_uid >= 0) ? unitig_l[mapped_uid].kmer_list.size() : (-1));
		}

		if(print_log){
			//show all rp
			fprintf(stderr, "read_id: %d\n", read_id);
			for (READ_PATH_ITEM &r : rp)
			{
				r.show();
			}
			fprintf(stderr, "\n");
		...
		{
			if (rp[i].kmer_number == 0 || rp[i].is_combined)
				continue;
			rp_com.emplace_back();
			rp_com.back().init(rp[i], read_id);
			for (uint j = i + 1; j < rp.size(); j++)
			{
				bool try_com = rp_com.back().combine(rp[j]);
				if (try_com == false)
					break;
			}
		}
		...
		if(print_log){
			//show all rp_com
			fprintf(stderr, "read_id: %d\n", read_id);
			for (READ_PATH_COMBINE_ITEM &r : rp_com)
			{
				r.show();
			}
			fprintf(stderr, "\n");
		...
		// get the total gap number
		{
			read_path.total_match_number = 0;
			for (READ_PATH_COMBINE_ITEM &r : rp_com)
			{
				read_path.total_match_number += r.match_kmer_num;
			}
		}
	}
						...
						std::vector<UNITIG_INFO_item> &unitig_l, bool showLog)
	{
		...
		for (unsigned j = 0; j < kmer_number; ++j)
		{
			int unitigPos;
			int unitigId = get_read_kmer_UNITIG_ID(j, seq, unitigPos, kmerIdx);
			if (mapped_uid != unitigId)
			{
				fprintf(stderr, "|%d(%d)--", mapped_uid, mapped_kmer_number);
				mapped_uid = unitigId;
				mapped_kmer_number = 1;
			}
			else
					  ...
					  std::vector<UNITIG_INFO_item> &unitig_l, bool showLog)
	{
		...
		for (unsigned strIndex = 0; strIndex < strCount; ++strIndex)
		{
			...
			unsigned kmer_number = seqLen - kmerSize + 1;
			if (showLog)
				fprintf(stderr, "read ID %d\t", strIndex);
			for (unsigned j = 0; j < kmer_number; ++j)
			{
...
struct READ_MATCH_NUM
{
	...
	void set(int read_id, int total_match)
	{
		this->read_id = read_id;
	...
	static inline int cmp_by_total_match(const READ_MATCH_NUM &a, const READ_MATCH_NUM &b)
	{
		if (a.is_full_cover != b.is_full_cover)
			return a.is_full_cover > b.is_full_cover;
		return a.total_match > b.total_match;
	}

	static inline int cmp_by_id(const READ_MATCH_NUM &a, const READ_MATCH_NUM &b)
	{
		return a.read_id < b.read_id;
	}
};

struct READ_PATH_HANDLER
{
	std::vector<READ_MATCH_NUM> read_match_num_l;
	std::vector<READ_PATH> read_path_list;

	void clear()
	{
		read_match_num_l.clear();
		read_path_list.clear();
	}

	void get_the_read_path(bool print_log, PathReadFilter &prFilter, int KMER_len,
						   std::unordered_map<std::string, DBG_INFO_item> &kmerIdx, std::vector<UNITIG_INFO_item> &unitig_l, std::vector<std::string> &reads_str)
	{
		int read_id = -1;
		for (std::string &r : reads_str)
		{
			read_id++;
			read_path_list.emplace_back();
			prFilter.get_string_path(print_log,kmerIdx, unitig_l, KMER_len, r, read_path_list.back(), read_id);
			read_match_num_l.emplace_back();
			read_match_num_l.back().set(read_id, read_path_list.back().total_match_number);
		}
		// show_all_read path();
		if (print_log)
		{
			int read_id = -1;
			fprintf(stderr, "###AATotal read number %ld\n", reads_str.size());
			for (std::string &r : reads_str)
			{
				read_id++;
				read_path_list[read_id].show_read_path(read_id, r.size());
			}
			fprintf(stderr, "###SIMPLE: Total read number %ld\n\n\n", reads_str.size());
			read_id = -1;
			for (std::string &r : reads_str)
			{
				read_id++;
				read_path_list[read_id].show_combine_read_path(read_id, r.size());
			}
		}
	}

	void sort_read_path_by_match_number()
	{
		std::sort(read_match_num_l.begin(), read_match_num_l.end(), READ_MATCH_NUM::cmp_by_total_match);
	}
	void restore_sort_read_path_by_id()
	{
		std::sort(read_match_num_l.begin(), read_match_num_l.end(), READ_MATCH_NUM::cmp_by_id);
	}
};
...
struct MainDBGHandler1
{
	...
	void addRef(bool print_log, uint8_t *tseq, int tlen, Contig_String_aligner *ca);
	int curKmerLen;
	...
	std::set<std::string> read_begin_kmer;
	std::set<std::string> read_end_kmer;
	...
	void init()
	{
		random_seed = get_random_seed_by_time();
		abH.init();
		gra.init();
	}
	void clear()
	{
		reads_str.clear();
		reference.clear();
		kmerCountingTable.clear();
		read_begin_kmer.clear();
		read_end_kmer.clear();
	}
	void showAllRead()
	{
	...
	/// wordCount: k-mer ==> number of reads containing the k-mer
	/// wordSupportReads: k-mer ==> a list of read IDs containg the k-mer
	void kmerCounting(const unsigned kmerSize, bool IsStoreReadInfo, bool using_reference)
	{
		Kmer_ANA k_a[3];
		for (int i = 0; i < 3; i++)
			k_a[i].clear();
		...
		for (int mode = 0; mode < 2; mode++)
		{
			...
			if (mode == 0)
				...
				c_reads = &(reads_str);
			// not using reference in the kmer counting
			if (!using_reference && mode == 0)
				continue;
			const unsigned strCount = c_reads->size();
			for (unsigned read_idx = 0; read_idx < strCount; ++read_idx)
			{
				// ref: KMER mode is 0;
				// SRS read: KMER mode is 1;
				// TGS read: KMER mode is 2;
				int kmer_mode = mode;
				if (mode == 1 && read_info[read_idx].is_LRS_read) // SRS read
					kmer_mode = 2;
				// stores the index of a kmer in a read sequence
				...
				for (unsigned kmer_idx = 0; kmer_idx < kmer_number; ++kmer_idx)
				{
					...
					if (kmer_idx == 0 && seqLen > 5000) // store the read begin kmer
						read_begin_kmer.emplace(word);
					if (kmer_idx == kmer_number - 1 && seqLen > 5000) // store the read begin kmer
						read_end_kmer.emplace(word);
					  ...
					  std::vector<Path> &pathList, std::vector<UNITIG_INFO_item *> &randomSearchBuff, int MAX_nodePassTime, int &ALL_NODE_USED)
	{
		...
		if (it == node_reach_count.end())
		{
			node_reach_count[curID] = 1;
		}
	...
	void D_searchPath_TGS(int headID, int curID, std::set<int> &stopNodeSet, std::vector<ReadPos> &cur_support_read_position_l,
						  std::vector<Path> &pathList, std::vector<UNITIG_INFO_item *> &randomSearchBuff)
	{
		bool stopAtThisNode = false;
		std::vector<ReadPos> ori_r_p_l = cur_support_read_position_l; // copy
		...
		if (curID == -1)
		{
			randomSearchBuff.emplace_back((UNITIG_INFO_item *)NULL);
			stopAtThisNode = true;
			cur_support_read_position_l.clear();
		}
		else
		{
			randomSearchBuff.emplace_back(&(unitig_l[curID]));
			...
			// TODO::
			std::vector<ReadPos> &TGS_pos = unitig_l[curID].getAllReadTGSHead(id2info);
			// show all the read TGS
			if (false)
			{
				for (ReadPos &p : TGS_pos)
					p.show(stderr);
			}
			// update the support read list:
			// B1: when the handled node is head node,init the read list;
			if (curID == headID)
			{
				cur_support_read_position_l = unitig_l[curID].getAllReadTGSTail(id2info);
			}
			// B2: when the handled node is middle node, check all reads and decide whether each read support the path
			else
			{
				// Double layer for-cycle to check support read
				// todo:: this method is slow, change to hash table to accelerate
				for (uint i = 0; i < cur_support_read_position_l.size(); i++)
				{
					ReadPos &rp = cur_support_read_position_l[i];
					bool truePath = false;
					...
					// when read not pass the filter, remove it from the read support list
					if (truePath == false)
					{
						rp.readID = -1;
						cur_support_read_position_l.erase(cur_support_read_position_l.begin() + i);
						i--;
						// todo::wrong reads will be added to the recover read list, when meet the following condition:
						// the reads is erased because sequencing error, not caused by branch: that is, kmer of read not appear in any UNITIGs
						//
					}
					// SNP and small gap recovering:
					// todo::
					// meet the two condition to recover a read:
					//(1)read under-go sequencing error;
					//(2)gap is small
				}
			}
			if (cur_support_read_position_l.empty())
				stopAtThisNode = true;
			else
			{
				...
				for (ReadPos &node_rp : cur_support_read_position_l)
				{
		...
		bool isFullPath = false;
		// stop when reach the node in the END list
		if (curID != headID && stopNodeSet.find(curID) != stopNodeSet.end() && !cur_support_read_position_l.empty())
		{
			stopAtThisNode = true;
			isFullPath = true;
		}
		...
		if (stopAtThisNode)
		{
			if (isFullPath)
			{
				...
				pathList.back().unitigPL = randomSearchBuff;				  // copy
				pathList.back().endUID = curID;								  // copy
				pathList.back().support_read_l = cur_support_read_position_l; // copy
			}
		}
		else
		{
			...
			{
				// check the branch:
				D_searchPath_TGS(headID, outID, stopNodeSet, cur_support_read_position_l, pathList, randomSearchBuff);
			}
		}
		// restore the read list
		std::swap(ori_r_p_l, cur_support_read_position_l); // copy
		randomSearchBuff.pop_back();
	}

	void getDBGPathLRS(bool print_log)
	{
		pc_l_final.clear();
		std::map<int, int> node_reach_count;
		// the map to store the current read ID and positions
		std::vector<ReadPos> cur_r_p;
		// for each KEY node, DO:
		int keyPosNum = keyPosUnitigL.size();
		for (int i = 0; i < keyPosNum - 1; i++)
		{
			UNITIG_INFO_item *up = keyPosUnitigL[i].p;
			...
			D_searchPath_TGS(beginNodeID, beginNodeID,
							 keyPosUnitigIDset, cur_r_p, pathList, randomSearchBuff);
			for (Path &p : pathList)
			{
				p.setSVLen(id2info);
				if (print_log)
	...
	// deepest-first search the DBG to find all the path in the DBG ONLY using TGS-reads
	void D_searchPath_TGS_ONT_HAC(int headID, int curID, std::set<int> &stopNodeSet, std::vector<ReadPos> &cur_support_read_position_l,
								  std::vector<ReadPos> &cur_removed_read_position_l,
								  std::vector<Path> &pathList, std::vector<UNITIG_INFO_item *> &randomSearchBuff)
	{
		bool stopAtThisNode = false;
		std::vector<ReadPos> ori_support_read_position_l = cur_support_read_position_l; // copy
		std::vector<ReadPos> ori_removed_read_position_l = cur_removed_read_position_l; // copy
		...
		if (curID == -1)
		{
			...
			stopAtThisNode = true;
			cur_support_read_position_l.clear();
			ori_removed_read_position_l.clear();
		}
		else
		{
			randomSearchBuff.emplace_back(&(unitig_l[curID]));
			...
			uint kmer_step = unitig_l[curID].kmer_list.size();
			if (curID == headID)
			{
				kmer_step = 1;
			}
			...
			if (curID == headID)
			{
				cur_support_read_position_l = unitig_l[curID].getAllReadTGSTail(id2info);	   // copy
				unitig_l[curID].getAllReadTGS_head_only(id2info, cur_removed_read_position_l); // copy
			}
			// B2: when the handled node is middle node, check all reads and decide whether each read support the path
			else
			{
				...
				// todo:: this method is slow, change to hash table to accelerate
				{
					fprintf(stderr, "curID is : %d \n\n BEFORE: SUP: ", curID);
					for (ReadPos &r : cur_support_read_position_l)
						r.show(stderr);
					fprintf(stderr, "\nBEFORE: REM: ");
					for (ReadPos &r : cur_removed_read_position_l)
						r.show(stderr);
					fprintf(stderr, "\n");
				}

				std::vector<ReadPos> new_remove;
				for (uint i = 0; i < cur_support_read_position_l.size(); i++)
				{
					ReadPos rp = cur_support_read_position_l[i]; // copy
					bool truePath = false;
					for (ReadPos &node_rp : TGS_pos)
					{
						// check the node ID
					...
					if (truePath == false)
					{
						...
						// todo::wrong reads will be added to the recover read list, when meet the following condition:
						// the reads is erased because sequencing error, not caused by branch: that is, kmer of read not appear in any UNITIGs
						// read is removed here:
						int unitig_ID, kmer_in_unitig_pos;
						get_read_KMER_UNITIG(rp.readID, rp.readPos, unitig_ID, kmer_in_unitig_pos);
						if (true)
						{
							fprintf(stderr, "REMOVE_read_check:curID %d, rp.readID %d, rp.readPos %d, unitig_ID %d, kmer_in_unitig_pos %d\n", curID, rp.readID, rp.readPos, unitig_ID, kmer_in_unitig_pos);
							if (kmer_in_unitig_pos == 0)
							{ // treat as other branch:
								fprintf(stderr, "REMOVE_read_check result: Other branch, remove\n ");
							}
							else if (unitig_ID == -1)
							{
								fprintf(stderr, "REMOVE_read_check result: ERROR sequencing, add to remove list\n ");
							}
							else
							{
								fprintf(stderr, "REMOVE_read_check result: CPX need re-check\n ");
							}
						}

						if (unitig_ID == -1 || kmer_in_unitig_pos != 0)
							new_remove.emplace_back(rp.readID, rp.readPos); //~~~
						else
						{ /*DO NOTHING*/
						}
					}
					else
					{
						if (true)
						{
							fprintf(stderr, "REMOVE_read_check:curID %d, rp.readID %d, rp.readPos %d, PASS and add to branch\n", curID, rp.readID, rp.readPos);
						}
					}
				}
				{
					...
					// cal gap::;
					for (uint i = 0; i < cur_removed_read_position_l.size(); i++)
					{
						ReadPos recover_try_rp = cur_removed_read_position_l[i]; // copy
						bool true_recovered = false;
						int recovered_gap_size = 0;
						int recovered_distance = 0; // todo::>?>?>>>>>>??????
						for (ReadPos &node_rp : TGS_pos)
						{
							// check the node ID
							if (recover_try_rp.readID == node_rp.readID)
							{
								if (ABS_U(recover_try_rp.readPos, node_rp.readPos) < 6)
								{ // gap of 6bp is allowed
									recovered_gap_size = recover_try_rp.readPos - node_rp.readPos;
									recover_try_rp.readPos = node_rp.readPos; // reset the position
									true_recovered = true;
									break;
								}
							}
						}
						// recovered the read and store to the support read list, the remove from removed read list
						if (true_recovered)
						{
							fprintf(stderr, "Truly read recovered, and the unitig id is %d, the read ID is %d the pos is %d , gap size is  %d \n", curID, recover_try_rp.readID, recover_try_rp.readPos, recovered_gap_size);
							cur_support_read_position_l.emplace_back(recover_try_rp.readID, recover_try_rp.readPos);
							cur_removed_read_position_l.erase(cur_removed_read_position_l.begin() + i);
							i--;
						}
					}
					for (ReadPos &r : new_remove)
						cur_removed_read_position_l.emplace_back(r);
				}
			...
			if (cur_support_read_position_l.empty() && cur_removed_read_position_l.empty())
				stopAtThisNode = true;
			else
			{
				// update read position for support reads
				for (ReadPos &node_rp : cur_support_read_position_l)
					node_rp.readPos += kmer_step;
				// update read position for removed reads
				for (ReadPos &node_rp : cur_removed_read_position_l)
					node_rp.readPos += kmer_step;
			}
		}
		...
		bool isFullPath = false;
		// stop when reach the node in the END list
		if (curID != headID && stopNodeSet.find(curID) != stopNodeSet.end() && !cur_support_read_position_l.empty() && !cur_removed_read_position_l.empty())
		{
			stopAtThisNode = true;
			isFullPath = true;
		}

		if (randomSearchBuff.size() > 50)
		{
			stopAtThisNode = true;
		}
		...
		if (stopAtThisNode)
		{
			if (isFullPath)
			{
				...
				pathList.back().endUID = curID;								  // copy
				pathList.back().support_read_l = cur_support_read_position_l; // copy
				pathList.back().removed_read_l = cur_removed_read_position_l; // copy
			}
		}
		else
		{
			...
			{
				// check the branch:
				D_searchPath_TGS_ONT_HAC(headID, outID, stopNodeSet, cur_support_read_position_l, cur_removed_read_position_l, pathList, randomSearchBuff);
			}
		}
		// restore the read list
		std::swap(ori_support_read_position_l, cur_support_read_position_l); // copy
		std::swap(ori_removed_read_position_l, cur_removed_read_position_l); // copy

		randomSearchBuff.pop_back();
	}

	// check whether the kmer in ‘read_pos’ of 'read_ID' appeared in one of UNITIG
	// return 'unitig_ID' and 'kmer_in_unitig_pos' when appeared
	// return -1 when not
	void get_read_KMER_UNITIG(int read_ID, int read_pos, int &unitig_ID, int &kmer_in_unitig_pos)
	{
		unitig_ID = prFilter.get_read_kmer_UNITIG_ID(read_pos, reads_str[read_ID], kmer_in_unitig_pos, kmerIdx);
	}

	void getDBGPath_ONT_HAC(bool print_log)
	{
		pc_l_final.clear();
		std::map<int, int> node_reach_count;
		// the map to store the current read ID and positions
		std::vector<ReadPos> cur_support_read_pos_l;
		std::vector<ReadPos> cur_removed_read_pos_l;
		// std::vector<ReadPos> cur_r_p;
		// for each KEY node, DO:
		int keyPosNum = keyPosUnitigL.size();
		for (int i = 0; i < keyPosNum - 1; i++)
		{
			...
			if (keyPosUnitigIDset.find(beginNodeID) == keyPosUnitigIDset.end())
				continue;
			cur_support_read_pos_l.clear();
			cur_removed_read_pos_l.clear();
			pathList.clear();
			node_reach_count.clear();
			randomSearchBuff.clear();
			// store the begin read list:
			D_searchPath_TGS_ONT_HAC(beginNodeID, beginNodeID,
									 keyPosUnitigIDset, cur_support_read_pos_l, cur_removed_read_pos_l, pathList, randomSearchBuff);
			for (Path &p : pathList)
			{
				p.setSVLen(id2info);
				if (print_log)
